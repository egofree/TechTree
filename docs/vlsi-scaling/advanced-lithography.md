...
> *Shows growing number of lithography steps for advanced nodes. Based on 11/24/2014 presentation by ASML's Martin van den Brink.*

> *Image: Guiding light, Public domain*

![Increasing litho steps for advanced nodes](../images/vlsi-scaling/vlsi-scaling_advanced-lithography.png)
...
|---------|---------------|----------|
| CD (Critical Dimension) variation exceeds spec across wafer | Focus drift from immersion water temperature instability; lens heating from prolonged exposure; non-uniform illumination | Check immersion water temperature controller (±0.01°C); run lens heating compensation update; verify illuminator uniformity (<0.5% variation); check wafer leveling sensor calibration |
| Overlay error exceeds ±3 nm (3σ) between layers | Stage grid calibration drift; alignment mark degradation from etch/dep processes; reticle registration error | Re-run stage grid calibration; inspect alignment marks under microscope — if degraded, modify mark design for process compatibility; verify reticle registration via die-to-database inspection; apply updated intrafield correction parameters |
| Excimer laser power dropping below spec | Gas mixture degradation (fluorine reacts with tube walls); discharge electrode erosion; optics contamination from gas byproducts | Replace laser gas (typical lifetime 1-5M pulses); inspect discharge electrodes for erosion; clean intracavity optics; verify gas purifier function; check high-voltage discharge circuit for arc instability |
| Immersion water bubbles during scan | Degassed water system leak introducing air; meniscus control failure at high scan speed; water filter clogged causing pressure transient | Check water degasser vacuum level; verify meniscus confinement at 600+ mm/s scan speed; replace DI water filters; inspect lens-wafer gap for particle obstructions disrupting water flow |
| EUV source power unstable or below target | Sn droplet targeting error (laser misalignment on droplets); CO₂ laser amplifier degradation; collector mirror reflectivity loss from Sn contamination | Verify Sn droplet generator stability (droplet timing, diameter 25-30 μm); realign CO₂ laser beam on droplet stream; check collector mirror reflectivity — if below 60%, schedule replacement; verify hydrogen gas flow for Sn debris mitigation (200-500 sccm) |
| Resist "T-topping" (undercut profile at top of feature) | Airborne amine contamination neutralizing photogenerated acid before PEB; chemical filter saturation in cleanroom air recirculation | Replace amine scrubbing filters on recirculation air handlers; verify amine concentration <1 ppb in litho bay; check that PEB delay time (exposure to bake) is within spec (<30 min); consider adding cover coat to resist process |
| Line-edge roughness (LER) exceeds 3 nm (3σ) | Shot noise from low dose (insufficient photons at EUV); resist development process variation; post-exposure bake temperature non-uniformity | Increase dose if throughput allows (LER ∝ 1/√dose); verify PEB plate temperature uniformity (<±0.1°C); check developer concentration (0.26N TMAH) and dispense uniformity; evaluate metal-oxide resist for EUV layers |
| Mask defect found after wafer exposure | Particle deposition on reticle during handling or storage; pellicle damage; reticle stage contamination | Inspect reticle on die-to-database tool; if defect is opaque (extra material), FIB repair may be possible (±5-10 nm accuracy); check pellicle integrity; review reticle storage environment (ISO Class 1, N₂ purge); clean reticle stage |
