# Pest Management

> **Node ID**: agriculture.pest-management
> **Domain**: [Agriculture](./index.md)
> **Dependencies**: [`agriculture`](./index.md), [`foundations.food-agriculture`](../foundations/food-agriculture.md)
> **Enables**: [`food-processing`](../food-processing/index.md), [`agriculture.crop-rotation`](crop-rotation.md)
> **Timeline**: Years 0-15+
> **Outputs**: crop_protection, pest_control_methods, beneficial_habitat
> **Critical**: No — crop losses to pests are significant (10-40%) but rarely cause total civilization-level failure


Pest management is the practice of preventing, reducing, or controlling damage to crops caused by insects, pathogens, weeds, birds, and mammals. In a bootstrapping civilization without synthetic pesticides, pest management relies on biological knowledge, physical barriers, cultural practices, and targeted use of naturally occurring substances. The goal is not pest elimination — that is neither achievable nor desirable — but keeping pest populations below economic threshold levels where the cost of damage exceeds the cost of control.

Unmanaged pest pressure causes 10-40% crop losses in temperate agriculture and 30-60% in tropical agriculture. Historical records of locust plagues, blight epidemics (Irish potato famine, 1845-1849), and stored grain infestations demonstrate that pests can collapse food systems. The integrated approach — combining multiple control tactics so that no single pest can overcome all defenses — is the most robust strategy for a civilization that cannot manufacture chemical pesticides.

Integrated Pest Management (IPM) integrates four categories of control: cultural (crop rotation, planting timing, sanitation), physical (barriers, traps, hand-picking), biological (predators, parasites, pathogens), and chemical (plant-derived insecticides, sulfur, mineral oils). The emphasis is on cultural and biological methods, with chemical controls reserved for outbreaks that exceed economic thresholds.

Position in the dependency chain: pest management depends on [Foundations → Agriculture](../foundations/food-agriculture.md) (basic crop knowledge) and supports [Crop Rotation](crop-rotation.md) (rotation is itself a pest management strategy) and [Food Processing](../food-processing/index.md) (protected crops reach processing with less damage and contamination).

## Prerequisites

**Materials**:
- Fiber cloth or netting for physical barriers (see [Textiles](../textiles/fibers.md))
- Wood, bamboo, and stakes for trap crop planting and barrier construction
- Sulfur (for fungicide dust) — from [Chemistry](../chemistry/index.md) or volcanic deposits
- Neem seeds, pyrethrum flowers, or other botanical insecticide plants (regionally available)
- Ash (for dusting stored grain and deterring crawling insects)

**Tools and equipment**:
- [Hand tools](../foundations/tools-basic.md) for weeding and trap crop management
- [Sickles and hoes](../foundations/tools-basic.md) for sanitation (removing crop residue)
- [Sprayer or duster](../foundations/tools-basic.md) (simple backpack or hand-pumped) for applying botanical sprays
- Sticky traps (coated boards or papers) for insect monitoring

**Knowledge**:
- Identification of common pest species and their natural enemies
- Understanding pest life cycles (egg → larva → pupa → adult for insects) to target vulnerable stages
- Economic threshold concept: the pest density at which control cost equals the value of damage prevented
- Companion planting principles: which plant combinations repel specific pests

**Infrastructure**:
- Diverse cropping system (monocultures are pest magnets)
- [Compost system](soil-management.md) for healthy soil that produces vigorous, pest-resistant plants
- Storage facilities with rodent-proofing and ventilation (see [Food Processing → Preservation](../food-processing/preservation.md))


## Companion Planting Border (100 m of Field Edge)

| Material | Quantity | Source | Alternatives |
|----------|----------|--------|-------------|
| Marigold seed | 50-100 g | [Seed Saving](seed-saving.md) | Calendula, tansy |
| Garlic cloves | 2-5 kg | [Seed Saving](seed-saving.md) | Chives, onions, leeks |
| Basil seed | 20-50 g | [Seed Saving](seed-saving.md) | Mint, rosemary (in warm climates) |
| Nasturtium seed | 30-60 g | [Seed Saving](seed-saving.md) | Other trap crop flowers |
| Wooden stakes (for support) | 50-100 | [Tools → Woodworking](../foundations/tools-basic.md) | Bamboo poles |

## Botanical Insecticide Spray (10 Liters)

| Material | Quantity | Source | Alternatives |
|----------|----------|--------|-------------|
| Neem seeds (crushed) or pyrethrum flowers | 200-500 g | Cultivated plants | Derris root, rotenone plants, tobacco leaves |
| Liquid soap (potash-based) | 20-50 mL | [Chemistry → Alkalis](../chemistry/alkalis.md) | Mild detergent, yucca root extract |
| Water | 10 L | [Water Procurement](../water/procurement.md) | — |
| Container (clay or wood) | 10-20 L | [Ceramics](../ceramics/pottery.md) | Wooden bucket |
| Fine cloth (for straining) | 0.5 m² | [Textiles](../textiles/fibers.md) | Layer of grass or sand filter |

## Physical Barrier (Row Cover, 100 m of Row)

| Material | Quantity | Source | Alternatives |
|----------|----------|--------|-------------|
| Lightweight spun fiber or fine netting | 100-150 m² | [Textiles](../textiles/fibers.md) | Cheesecloth, reed matting |
| Wire or bamboo hoops (supports) | 50-100 pieces | [Tools](../foundations/tools-basic.md) | Flexible willow whips |
| Stones or sandbags (to seal edges) | 100-200 | On-site | Earth from furrows |
| Twine or wire (to secure) | 100-200 m | [Textiles](../textiles/fibers.md) | Willow withes |


## Cultural Controls

Cultural controls modify the farming environment to make it less hospitable to pests. These are the first line of defense and require no additional materials.

1. **Crop rotation**: Rotate crop families (not just species) between fields each year. Many soil-borne pathogens and insect pests are host-specific — they cannot persist without their preferred host. A 3-4 year rotation breaks most pest cycles. See [Crop Rotation](crop-rotation.md) for detailed rotation planning.
2. **Sanitation**: Remove and destroy crop residues after harvest. Plow under or compost all plant debris. Many pests overwinter in crop residue — removing it eliminates their shelter. Burn heavily infested material (do not compost diseased plants).
3. **Planting timing**: Plant before or after peak pest emergence. Early-planted wheat avoids Hessian fly emergence. Late-planted sweet corn avoids corn earworm peak. Know the emergence timing of key local pests and adjust planting dates by 2-4 weeks.
4. **Trap cropping**: Plant a small area (5-10% of main crop) of a preferred host 1-2 weeks before the main crop. Pests concentrate on the trap crop, which is then destroyed (pulled and burned, or heavily treated with botanical spray). Example: plant mustard trap crop 2 weeks before cabbage to attract flea beetles.
 5. **Resistant varieties**: When saving seed (see [Seed Saving](seed-saving.md)), select plants that showed pest resistance. Over generations, this builds locally adapted, pest-resistant crop populations.

**Strengths**:
- No additional materials or cost — modifies existing farming practices only
- Crop rotation breaks pest cycles for 80-95% of soil-borne pathogens and host-specific insects
- Builds long-term soil health and pest resistance rather than treating symptoms reactively

**Weaknesses**:
- Requires advance planning (rotation maps, planting schedules) and discipline across multiple seasons
- Trap cropping sacrifices 5-10% of planted area and requires timely destruction of the trap crop
- Sanitation removes beneficial insect habitat along with pest sheltering sites

## Physical Controls

1. **Hand-picking**: For large, visible pests (caterpillars, beetles, slugs), hand-pick daily during peak activity (early morning for slugs, mid-morning for caterpillars). Drop into a bucket of soapy water. Effective for small plots (0.1-0.5 ha). Labor-intensive but requires zero materials.
2. **Barriers — row covers**: Drape lightweight fabric or netting over wire hoops placed along crop rows. Bury or weigh down edges to exclude flying insects. Remove covers when plants flower (to allow pollination). Row covers reduce cabbage moth damage by 80-95%, carrot rust fly by 90%+.
3. **Barriers — collars**: Place 5-10 cm diameter collars of cardboard, tin, or bark around transplant stems at soil level. Collars prevent cutworms from reaching stems. Effective and low-effort for transplanted crops (cabbage, tomato, pepper).
4. **Sticky traps**: Coat yellow boards or cards with sticky material (pine resin + oil, or purchased Tanglefoot). Hang at crop height. Aphids, whiteflies, and thrips are attracted to yellow and become trapped. Use 10-20 traps per 100 m² for monitoring; 50-100 for mass trapping.
 5. **Water spray**: A strong jet of water from a hose or pump dislodges aphids and mites from plant leaves. Repeat every 2-3 days. Does not kill all pests but reduces populations below damage thresholds on sturdy crops.

**Strengths**:
- Row covers provide 80-95% protection from flying insects with zero chemical input
- Hand-picking is immediately effective and requires no materials — suitable for small plots
- Sticky traps provide quantitative monitoring data for pest population tracking

**Weaknesses**:
- Labor-intensive at scale — hand-picking requires 15-30 minutes per 100 m² daily during peak pest season
- Row covers must be removed at flowering for pollination, creating a vulnerability window
- Physical barriers exclude beneficial insects along with pests, disrupting biological control

## Biological Controls

1. **Encourage beneficial insects**: Plant nectar and pollen sources (umbellifers: dill, fennel, coriander; composites: yarrow, daisies) around field edges. Adult parasitic wasps and hoverflies feed on nectar; their larvae prey on or parasitize crop pests. A border planting 1-2 m wide around each field increases beneficial insect populations 2-5×.
2. **Ladybug conservation**: Ladybugs (Coccinellidae) eat 50-100 aphids per day per adult. Avoid spraying broad-spectrum botanical insecticides when ladybugs are active. Provide overwintering habitat: piles of rocks, leaf litter at field edges, wooden shelters with bark.
3. **Lacewing habitat**: Green lacewings (Chrysopidae) larvae consume 200-300 aphids per week. Adults are attracted to light-colored flowers. Plant alyssum, buckwheat, or coriander as lacewing attractants.
4. **Poultry for pest control**: Allow chickens or ducks to forage in fields after harvest. Poultry consume insects, weed seeds, and slugs at rates of 50-200 g animal feed equivalent per bird per day. Time poultry access to avoid crop damage — before planting or after harvest.
 5. **Predatory mites**: In fruit trees and vines, predatory mites (Phytoseiulus, Amblyseius) control spider mites. Introduce by placing mite-infested leaves from healthy ecosystems onto affected plants. Establish if broad-spectrum sprays are avoided.

**Strengths**:
- Ladybug adults consume 50-100 aphids per day — a single ladybug population can control aphids across 50-100 m²
- Beneficial insect borders (1-2 m wide) increase predator populations 2-5× with minimal land sacrifice
- Poultry foraging after harvest destroys 60-80% of overwintering pests and weed seeds

**Weaknesses**:
- Biological control is slow to establish — 2-4 weeks from habitat planting to measurable pest reduction
- Broad-spectrum botanical sprays (pyrethrum) kill beneficial insects along with pests, undermining the system
- Predatory mites and parasitic wasps require specific habitat conditions (humidity, shelter) that may not exist in all field margins

## Botanical Insecticide Preparation and Application

**Neem spray** (broad-spectrum, repels and disrupts insect growth):

1. **Harvest seeds**: Collect fallen neem fruit. Remove pulp, dry seeds in sun 3-5 days. Shell to extract kernels.
2. **Prepare extract**: Crush 200-500 g neem kernels. Soak in 10 L water for 12-24 hours at room temperature. Strain through fine cloth.
 3. **Add emulsifier**: Mix 20-50 mL liquid soap into the filtered extract. Soap helps the neem oil disperse and stick to plant surfaces.
4. **Apply**: Spray onto plant leaves (both upper and lower surfaces) in early morning or late evening (avoid midday heat, which causes leaf burn). Apply at first sign of pest damage. Repeat every 7-14 days as needed.
5. **Effective against**: Aphids, whiteflies, caterpillars, beetles, leafhoppers, thrips. Less effective against mites. Disrupts insect hormone systems — larvae fail to molt, eggs fail to hatch.

**Strengths (neem spray)**:
- Disrupts insect growth hormones at multiple life stages (eggs, larvae, adults) — resistance develops slowly
- Low mammalian toxicity (LD50 >5,000 mg/kg in rats) — safer to handle than synthetic pesticides
- Repels as well as kills — treated plants become unpalatable to pests for 7-14 days

**Weaknesses (neem spray)**:
- Requires 200-500 g neem kernels per 10 L — substantial seed harvest dedicated to pest control rather than other uses
- Degrades in UV light within 48-72 hours — must be reapplied frequently
- Concentrated neem oil causes contact dermatitis in some individuals

**Pyrethrum spray** (fast-acting contact insecticide):

1. **Harvest flowers**: Pick pyrethrum (Chrysanthemum cinerariaefolium) flowers in full bloom on a dry day. Dry in shade (sunlight degrades pyrethrins).
2. **Prepare dust or extract**: Grind dried flowers to fine powder. For spray: steep 50-100 g powder in 10 L water for 24 hours, strain. For dust: mix powder 1:5 with fine clay or talc.
 3. **Apply**: Spray or dust onto infested plants. Pyrethrum degrades rapidly in sunlight (half-life 12 hours) — apply in late afternoon for maximum overnight effectiveness. Repeat every 3-5 days for persistent infestations.
4. **Effective against**: Most insects (broad-spectrum). Kills on contact by disrupting nerve signals. Non-selective — also kills beneficial insects. Use only for spot-treatment of severe outbreaks, not as routine preventative.

**Strengths (pyrethrum spray)**:
- Fast-acting — kills insects on contact within minutes by disrupting nerve cell sodium channels
- Degrades rapidly in sunlight (half-life 12 hours) — no persistent residue on crops or in soil
- Can be produced locally from pyrethrum daisy flowers with minimal processing (dry, grind, steep)

**Weaknesses (pyrethrum spray)**:
- Non-selective — kills beneficial insects (ladybugs, lacewings, parasitic wasps) at the same rate as pests
- Highly toxic to fish and aquatic invertebrates — cannot be used near waterways
- UV degradation means short effective window — must be applied in late afternoon and repeated every 3-5 days


## Economic Threshold Levels (ETL) for Key Pests

| Pest | Crop | ETL (pests per plant or per sample) | Damage at ETL | Control Action |
|------|------|-------------------------------------|--------------|---------------|
| Aphids (cereal) | Wheat, barley | 5-10 per tiller, or 50% tillers infested | 5-10% yield loss | Botanical spray or ladybug release |
| Cabbage white butterfly | Cabbage, broccoli | 1-2 caterpillars per plant | 5-15% leaf area consumed | Hand-pick, Bt spray, or row cover |
| Colorado potato beetle | Potato | 4 adults or 15 larvae per plant | 10-20% defoliation | Hand-pick, neem spray, or crop rotation |
| Codling moth | Apple, pear | 2-3% fruit damage (monitor with traps) | Larvae in fruit | Pheromone traps, corrugated band traps |
| Slugs | Lettuce, cabbage | 1-3 per m² (trap count) | 5-15% crop damage | Beer traps, diatomaceous earth, duck foraging |
| Weevils (stored grain) | Stored wheat, rice | 2-5 per kg grain sample | 5-10% grain loss | Clean storage, diatomaceous earth, temperature control |
| Root-knot nematode | Tomato, carrot | 2-5 galls per root system | 10-30% yield loss | Marigold rotation, soil solarization |
| Flea beetles | Brassicas, eggplant | 10-20% leaf area damage (seedlings) | Stunting, reduced vigor | Row covers, trap crop, neem spray |

## Beneficial Insect Effectiveness

| Beneficial | Prey | Consumption Rate | Attractant Plants | Optimal Habitat |
|-----------|------|-----------------|-------------------|----------------|
| Ladybug (adult) | Aphids | 50-100/day | Dill, fennel, yarrow | Rock piles, leaf litter |
| Ladybug (larva) | Aphids | 20-40/day | Same as adult | Crop canopy |
| Lacewing (larva) | Aphids, mites, thrips | 200-300/week | Alyssum, coriander, buckwheat | Sheltered field edges |
| Hoverfly (larva) | Aphids | 30-50/day | Alyssum, dill, buckwheat | Open, sunny areas |
| Parasitic wasp (Trichogramma) | Moth eggs | 10-20 eggs/day | Flowering herbs, yarrow | Undisturbed borders |
| Ground beetle | Slugs, cutworms, root maggots | 5-15/day | Ground cover, leaf mulch | Stone walls, log piles |
| Predatory mite (Phytoseiulus) | Spider mites | 5-10/day | — | Humid, protected foliage |

## Companion Plant Spacing and Effectiveness

| Companion Pair | Pest Targeted | Spacing | Reduction in Pest Damage |
|---------------|--------------|---------|-------------------------|
| Tomato + basil | Whitefly, aphids | 30-50 cm between plants | 30-50% reduction |
| Cabbage + marigold | Cabbage root fly, nematodes | Marigold every 3rd cabbage | 40-60% reduction |
| Carrot + onion | Carrot rust fly, onion fly | Alternating rows, 20 cm apart | 60-80% reduction (mutual benefit) |
| Beans + summer savory | Bean beetle | Savory every 2-3 bean plants | 30-40% reduction |
| Squash + nasturtium | Squash bug, aphids | Nasturtium border, 30 cm spacing | 40-60% (trap crop effect) |
| Potato + horseradish | Colorado potato beetle | Horseradish at row ends | 20-30% reduction |
| Corn + beans + squash | Multiple (Three Sisters) | Interplanted, 30-50 cm spacing | 20-40% overall reduction |

## Trap Crop Effectiveness

| Main Crop | Trap Crop | Timing | Pests Trapped | % Pest Diverted from Main Crop |
|-----------|-----------|--------|--------------|-------------------------------|
| Cabbage | Mustard (Indian mustard) | Plant 2 weeks earlier | Flea beetles, diamondback moth | 60-80% |
| Tomato | Dill or tall marigold | Plant along border | Tomato hornworm, whitefly | 40-60% |
| Cotton | Okra or hibiscus | Plant along edges | Boll weevil | 50-70% |
| Beans | Alfalfa | Plant in strips | Lygus bug | 40-60% |
| Fruit trees | Zinnias | Plant at orchard edges | Japanese beetle | 50-70% |

## Scaling Notes

**Household garden (0.05-0.2 ha)**: Hand-picking, companion planting, and row covers are sufficient. Monitor plants daily during growing season. One person can manage all pest control for a household garden in 15-30 minutes per day. This is the scale where pest management is most effective — individual attention to each plant is possible.

**Small field (0.5-2 ha)**: Cultural controls (rotation, sanitation, trap cropping) become primary. Physical barriers protect high-value crops. Botanical sprays reserved for outbreaks exceeding economic thresholds. Beneficial insect habitat planted at field edges. Labor: 2-5 hours per week during peak pest season.

**Village agriculture (10-50 ha)**: Area-wide pest management coordinated across multiple fields. Synchronized planting dates reduce pest windows. Communal beneficial insect habitat corridors connect fields. Trap crops planted strategically across the landscape. Monitoring with sticky traps at standardized density (1 trap per 100 m²) provides early warning. Poultry foraging rotated across fields post-harvest.

**Regional scale (100+ ha)**: Landscape-level habitat management for beneficial insects (hedgerows, wildflower strips, beetle banks). Crop diversification across the region ensures no single pest can build epidemic populations. Pest forecasting based on weather data (temperature and rainfall predict insect emergence). This scale enables biological control programs (mass-rearing and releasing parasitic wasps, ladybugs).

**Key bottleneck**: Monitoring labor. Effective pest management requires regular scouting (walking the fields, inspecting plants). At larger scales, this requires dedicated personnel. Without scouting, outbreaks are detected too late for cultural or biological controls, forcing reactive chemical treatment.

## Troubleshooting

| Problem | Probable Cause | Solution |
|---------|---------------|----------|
| Botanical spray not working | UV degradation (applied in sun), incorrect concentration, or resistant pest population | Apply sprays in late afternoon; increase concentration by 50%; rotate between neem and pyrethrum to prevent resistance |
| Row covers trapping heat | Covers left on during hot weather (>30°C) | Remove covers when temperature exceeds 28°C; use taller hoops for better ventilation; switch to netting with larger mesh |
| Beneficial insects not establishing | No nectar/pollen sources, or broad-spectrum sprays killing them | Plant border flowers (alyssum, dill, buckwheat); eliminate broad-spectrum sprays; provide water sources (shallow dishes with pebbles) |
| Trap crop not attracting pests | Wrong species, wrong timing, or trap crop healthier than main crop | Use highly attractive trap crop species; plant 1-2 weeks before main crop; ensure trap crop is at attractive growth stage when pests emerge |
| Stored grain infestation | Grain stored above 15% moisture, or storage container not sealed | Dry grain to 8-12% moisture; seal containers; mix diatomaceous earth at 1 kg per tonne grain; freeze grain for 48 hours to kill eggs |
| Neem spray causing leaf burn | Concentration too high, or applied during hot sun | Dilute to 2-5% neem extract; apply in evening; test on 3-4 leaves first, wait 24 hours before spraying full crop |
| Slug problem worsening | Excessive mulch providing habitat, or nocturnal irrigation keeping surface wet | Reduce mulch thickness near susceptible crops; irrigate in early morning so surface dries by night; set beer traps (shallow dishes of beer at soil level) |
| Bird damage to fruit/seeds | No physical exclusion, or attractive crop near roosting sites | Net fruit trees with bird netting; use scare devices (reflective tape, predator silhouettes) moved every 3-5 days; plant sacrificial crop nearby |

## Safety

- **Pyrethrum toxicity to fish**: Pyrethrum is highly toxic to fish and aquatic invertebrates. Do not spray near waterways, ponds, or streams. Pyrethrum degrades rapidly in sunlight (half-life 12 hours), so treated areas are safe for most organisms within 24 hours.
- **Neem oil skin irritation**: Concentrated neem oil causes contact dermatitis in some individuals. Wear gloves when preparing neem extract. Diluted spray is generally safe for skin contact.
- **Sulfur dust inhalation**: Sulfur dust applied as fungicide irritates lungs. Wear a cloth mask over nose and mouth during application. Apply on calm days to avoid drift. Do not apply when temperatures exceed 35°C — sulfur can burn plant leaves.
- **Rotenone toxicity**: Rotenone (from derris root) is toxic to fish and moderately toxic to mammals. Use with caution. Do not apply near water. Rotenone has been linked to Parkinson-like symptoms in animal studies with chronic exposure — limit use to occasional spot-treatment.
- **Diatomaceous earth (DE) inhalation**: DE is abrasive silica. Breathing DE dust damages lung tissue. Wear a mask when applying. Use food-grade DE only (pool-grade DE is more hazardous). Apply with a duster, not by tossing handfuls.
- **Botanical spray eye contact**: All botanical sprays (neem, pyrethrum, garlic-chili) cause eye irritation. Avoid spraying toward face. Rinse eyes with clean water for 15 minutes if contact occurs.


## Pest Monitoring Standards

| Monitoring Tool | Placement Density | Frequency | What to Record | Action Threshold |
|----------------|-------------------|-----------|----------------|-----------------|
| Sticky trap (yellow) | 1 per 100 m² for monitoring | Check weekly | Pest count per trap, species identification | Species-specific (see ETL table above) |
| Pheromone trap | 1 per 500-1,000 m² | Check twice weekly | Target species count | 5-10 moths per trap per week (varies by species) |
| Visual scouting | Walk field in V-pattern, inspect 20 plants at random | Weekly during growing season | % plants with damage, pest species present | Damage exceeds 10% of leaf area or ETL reached |
| Beat sheet (for cereals) | 10 samples per field | Weekly during stem extension | Aphids, beneficials per beat | 5-10 aphids per tiller or 50% tillers infested |
| Trap crop inspection | Every 2-3 days when pests are active | Ongoing during pest window | Pest density on trap crop vs. main crop | Destroy trap crop when pest concentration exceeds 5× main crop |

## Stored Product Protection Verification

| Parameter | Target | Test Method |
|-----------|--------|-------------|
| Grain moisture at storage | 8-12% | Bite test (cracks cleanly) or moisture meter |
| Temperature of stored grain | <20°C (ideally 10-15°C) | Probe thermometer inserted into grain mass |
| Insect presence | 0 live insects per kg sample | Sieve 1 kg sample through 2 mm mesh; count insects |
| Rodent contamination | No droppings in grain sample | Visual inspection of grain surface and container perimeter |
| Diatomaceous earth coverage | 1 kg per tonne grain, evenly mixed | Dust visible on all grain surfaces when scooped |


## Pest Management Strategy by Scale

| Scale | Primary Strategy | Secondary | Chemical Use |
|-------|-----------------|-----------|-------------|
| Garden (0.1 ha) | Hand-picking, companion planting | Row covers, water spray | Botanical spot-treatment only |
| Small field (1 ha) | Cultural controls, beneficial habitat | Trap crops, physical barriers | Botanical sprays at ETL |
| Village (10-50 ha) | Rotation, sanitation, beneficial corridors | Area-wide monitoring, poultry foraging | Botanicals for outbreaks only |
| Regional (100+ ha) | Landscape habitat management, crop diversification | Biological control releases, forecasting | Minimal; chemical as last resort |

## Natural Pesticide Plants by Region

| Region | Plant | Active Compound | Target Pests | Preparation |
|--------|-------|----------------|-------------|-------------|
| South/Southeast Asia | Neem (Azadirachta indica) | Azadirachtin | Broad-spectrum (200+ species) | Seed kernel extract |
| East Africa | Pyrethrum (Chrysanthemum cinerariaefolium) | Pyrethrins | Broad-spectrum contact | Flower dust or extract |
| South America | Lonchocarpus (cube root) | Rotenone | Beetles, caterpillars, aphids | Root powder in water |
| Caribbean/Mexico | Gliricidia sepium | Coumarin derivatives | Leaf-cutter ants, termites | Leaf extract |
| Mediterranean | Lavender, rosemary | Essential oils | Moths, fleas, mosquitoes | Oil extract or dried bundles |
| Temperate worldwide | Garlic | Allicin, diallyl disulfide | Aphids, cabbage moth, nematodes | Crushed cloves in water |
| Tropical worldwide | Tobacco | Nicotine | Aphids, thrips, caterpillars | Leaf soak (handle with care — toxic) |
| Worldwide | Hot chili | Capsaicin | Mammals (deer, rabbits), some insects | Powdered or soaked in water |

## Physical Barrier Options by Crop

| Crop | Barrier Type | Installation Effort | Cost | Effectiveness | Notes |
|------|-------------|--------------------|------|--------------|-------|
| Brassicas (seedlings) | Floating row cover | Low | Low | 90-95% | Remove at flowering |
| Carrots | Fine mesh fence (60 cm tall) | Medium | Medium | 80-90% | Buried 5 cm at base |
| Fruit trees | Bird netting | Medium | Medium | 80-95% | Secure at trunk to exclude |
| Beans | Chicken wire cylinder | Medium | Low | 60-80% | Prevents rabbit damage |
| Stored grain | Sealed containers | Low | Low | 90-99% | Glass, clay, or metal with tight lids |
| Root crops | Underground wire mesh | High | High | 70-90% | Prevents vole/mole damage |

## See Also

- [Foundations → Agriculture & Food Production](../foundations/food-agriculture.md) — basic cultivation, harvest, and storage
- [Agriculture → Crop Rotation](crop-rotation.md) — rotation as primary pest management strategy
- [Agriculture → Seed Saving](seed-saving.md) — selecting pest-resistant varieties
- [Agriculture → Soil Management](soil-management.md) — healthy soil produces pest-resistant plants
- [Agriculture → Irrigation](irrigation.md) — water management affects pest pressure
- [Food Processing → Preservation](../food-processing/preservation.md) — stored product pest control
- [Animals → Poultry](../animals/poultry.md) — poultry for biological pest control in fields
- [Chemistry → Alkalis](../chemistry/alkalis.md) — soap production for spray emulsifiers
- [Plants → Edible Plants](../plants/edible-plants.md) — plant species with pest-repellent properties



[← Back to Agriculture](index.md)
