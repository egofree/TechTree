...
|---------|---------------|----------|
| CMOS IC destroyed during handling — gate oxide ruptured | ESD discharge ≥100V damaged gate oxide; CMOS inputs protected by diode clamps but still vulnerable without proper ESD precautions | Use grounded wrist straps and ESD mats; store ICs in conductive foam or antistatic bags; never insert or remove ICs with power applied; verify wrist strap continuity daily |
| CMOS latchup — IC shorts VDD to ground, draws excessive current, destroys chip | Supply voltage exceeded maximum (7V for 74HC, 15V for 4000 series) triggering parasitic thyristor in CMOS structure; or input voltage driven below ground or above VDD | Use current-limited bench supplies (100–200 mA) during prototyping; never apply input signals before VDD is stable; add input clamp diodes if signals can overshoot rails |
| D flip-flop output oscillates or settles to mid-voltage (metastability) | Setup time (1–3 ns in 180nm) or hold time (0.5–1 ns) violated — data changed during the sampling window around the clock edge | Ensure data is stable for setup time before and hold time after the active clock edge; for asynchronous inputs crossing clock domains, add a two-stage synchronizer (back-to-back flip-flops) to reduce metastability probability |
| Ripple carry adder produces incorrect sum at high clock frequency | Carry propagation delay exceeds clock period — 32-bit adder at ~2 gate delays per stage = 64 gate delays worst case; at 10 ns per stage = 320 ns total | Replace with carry-lookahead adder (O(log N) delay — ~6 gate delays for 32 bits); or pipeline the adder into 8-bit stages with intermediate register stages; reduce clock frequency below worst-case carry propagation time |
| Clock skew causes flip-flop to capture wrong data | Clock arrives at different flip-flops at different times due to unequal wire lengths; in 5 GHz processor (200 ps period), skew >20 ps causes hold time violation | Implement balanced clock tree (H-tree or mesh topology); match wire lengths from clock buffer to each flip-flop; use clock deskew circuits with adjustable delay elements; verify skew with oscilloscope time-domain measurements |
| TTL circuit works on breadboard but fails on PCB — ringing, glitches | Long, unterminated wires on breadboard cause reflections and ground bounce; TTL 1.2V noise margin insufficient with >1V ringing from fast edge rates (10 ns transitions) | Keep wire lengths short (<10 cm for TTL); add 0.1 μF decoupling capacitors within 1 cm of each IC's VCC pin; use a ground plane on PCB; for 74LS at 9 ns delay, treat interconnects as transmission lines above 15 cm |
| GAL (22V10) programming fails — device will not verify after write | EEPROM write voltage insufficient; programmer timing does not match device requirements; or device previously locked by security fuse | Verify programmer supplies correct VPP programming voltage; check programming algorithm matches manufacturer datasheet timing; if security fuse blown, device is permanently locked and must be replaced |
| DRAM system loses data intermittently — random bit errors | Refresh cycle not meeting 64 ms requirement — each row must be read and rewritten within 64 ms or charge leaks away (junction leakage time constant ~10–100 ms); or refresh cycles steal too much bus bandwidth | Verify refresh controller activates all rows within 64 ms window; check for refresh starvation during long burst accesses; test with reduced access patterns to isolate refresh vs. access errors; consider ECC (error-correcting code) for single-bit error detection and correction |
| CMOS logic draws excessive static power (should be ~0) | Input pin floating — CMOS input at undefined voltage causes both PMOS and NMOS to conduct simultaneously, creating a DC path from VDD to ground; one floating input can dissipate 10+ mW | Never leave CMOS inputs unconnected; tie all unused inputs to VDD or GND via pull-up/pull-down resistors (10–100 kΩ); verify PCB layout connects every input pin |
| PAL 16R8 design does not fit — too many product terms per output | Design requires more than 4–8 product terms per output macrocell; PAL has fixed OR array — cannot add more AND terms beyond the hardware limit | Simplify Boolean expressions using Karnaugh maps or Quine-McCluskey minimization; split complex functions across multiple PAL outputs with feedback; migrate to GAL 22V10 (more macrocells) or FPGA (LUT-based, virtually unlimited terms) |
