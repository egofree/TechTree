# Economics & Organization

![Ming Gold Coins (14153035681)](../images/economics-organization/economics-organization_hero.jpg)

> *Manchu Gallery, Liaoning Museum, Shenyang. Complete indexed photo collection at WorldHistoryPics.com.*

> *Image: Gary Todd from Xinzheng, China, CC0*

Capabilities in this domain:

<!-- TODO: source image for Division of Labor -->
- [Division of Labor](division-of-labor.md) — Specialization of workers into distinct trades and coordination of their interdependent tasks. Emerges from food surplus: when not everyone must farm, specialists can focus on metallurgy, pottery, construction, or tool-making. Foundation of all complex production.
<!-- TODO: source image for Trade & Barter -->
- [Trade & Barter](trade.md) — Exchange of surplus goods between specialists through barter and market systems. Creates the economic feedback loop: specialization → surplus → exchange → further specialization. Enables surplus circulation across communities.
![Currency & Standardized Exchange](../images/economics-organization/economics-organization_currency.jpg)
> *Image: Unknown authorUnknown author, Public domain*

- [Currency & Standardized Exchange](currency.md) — Standardized exchange media (commodity money, metal coinage) and units of account that solve barter's double-coincidence problem. Requires reliable metallurgy and standardized weights.
![Accounting & Record-Keeping](../images/economics-organization/economics-organization_accounting.jpg)
> *Image: Bolton &amp; Dickens via NYHS, Public domain*

- [Accounting & Record-Keeping](accounting.md) — Systematic recording of production inputs, trade transactions, and resource inventories. Emerges when economic activity exceeds mental tracking capacity (~50 specialists). Provides information infrastructure for rational decision-making.
<!-- TODO: source image for Supply Chain Coordination -->
- [Supply Chain Coordination](supply-chain.md) — Planning and management of multi-step production sequences, procurement from diverse sources, and inventory management across distributed operations. Essential when production involves >3 sequential specialist steps.
<!-- TODO: source image for Governance & Institutional Design -->
- [Governance & Institutional Design](governance.md) — Decision-making structures for large-scale resource allocation, dispute resolution, and collective action. Includes guild structures, cooperative governance, and administrative hierarchies coordinating hundreds of workers.

[↑ Back to Tech Tree](../index.md)
