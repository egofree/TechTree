...
|---------|---------------|----------|
| Incorrect instruction execution | Microcode ROM error or hardwired control logic bug | Single-step through instruction micro-operations using logic analyzer on control signals; compare control signal assertions against specification at each clock cycle |
| Memory read returns wrong data | Timing violation: address not stable before read strobe (setup time violation) | Insert wait states; check address setup time against memory access time (e.g., 10 ns SRAM requires address stable 5 ns before /OE assertion); verify trace propagation delay (<0.15 ns/mm on FR-4) |
| Processor hangs after branch | Branch target calculation error (off-by-one in PC update) | Verify branch offset encoding (relative vs. absolute); check PC increment logic increments by instruction width (2 or 4 bytes, not 1); test with branch-to-self loop for isolation |
| Cache miss rate unexpectedly high | Conflict misses in direct-mapped cache (two hot addresses mapping to same set) | Increase associativity (2-way or 4-way); increase cache size; align frequently-accessed data to different cache sets using compiler directives or manual padding |
| Bus contention — data corruption | Two devices driving bus simultaneously (bus arbitration failure) | Verify bus arbitration logic; check DMA request/grant handshake timing (grant must assert after request, devices must tri-state within 1 clock of grant removal); add bus keeper resistors (10-100 kΩ) to prevent floating |
| Interrupt lost during processing | Interrupt signal shorter than interrupt sampling period (edge-sensitive latch missed the pulse) | Edge-triggered latch on interrupt input with minimum pulse width >1 clock period; increase interrupt sampling rate; use level-sensitive interrupts where the requesting device holds the line until serviced |
| Pipeline data hazard corrupts result | Forwarding path missing or incorrect (RAW dependency not resolved) | Verify forwarding multiplexers route ALU result from EX/MEM pipeline register back to EX stage input; add pipeline stall (bubble injection) for load-use hazards (1 cycle penalty); test with dependent instruction sequences (ADD R1,R2,R3 followed immediately by ADD R4,R1,R5) |
| DRAM refresh failure causes data loss | Refresh counter stopped or refresh interval exceeds specification (64 ms max between row refreshes) | Verify refresh request generated every 7.8 μs (for 8192 rows in 64 ms); check refresh timer clock source; measure DRAM RAS-only refresh cycle timing against datasheet specifications |
| System hangs under heavy I/O load | DMA starving CPU of bus cycles (bus grant held too long) | Implement maximum DMA burst length (8-16 transfers); add fairness algorithm to bus arbiter; verify CPU cache hit rate during DMA activity (should remain >85% if cache is large enough) |
| Clock jitter causes metastability | Asynchronous input crosses clock domain without synchronization (setup/hold violation) | Add two-stage flip-flop synchronizer on all asynchronous inputs (reduces metastability failure rate to ~10⁻⁶ per year at 100 MHz); check that synchronized signals meet setup time of destination domain clock |

## Safety

- **High-speed clock signals**: A 100 MHz clock has 3.3 ns edges with significant high-frequency content (harmonics up to 500+ MHz). Use controlled-impedance PCB traces (50 Ω for single-ended, 100 Ω differential) for clock distribution. Unterminated clock lines cause reflections that produce double-clocking (flip-flops trigger twice per cycle). Terminate clock lines with series resistors (22-33 Ω) at the source or parallel termination (50 Ω to Vtt = 0.75 V for SSTL) at the destination. Clock trace length mismatch >50 ps (10 mm on FR-4) causes skew that degrades setup/hold margins. For 400+ MHz designs, use differential clock distribution (LVDS: 350 mV swing, 100 Ω differential impedance).
- **Power supply sequencing**: Multi-voltage designs (e.g., 3.3 V I/O with 1.2 V core) require power-up sequencing. If the I/O ring powers up before the core, the I/O drivers may attempt to drive into an unpowered core, causing latchup (parasitic thyristor firing, current >1 A, destroying the IC). Apply core voltage first, then I/O voltage. Reverse on power-down. Use a power sequencing IC or supervisory circuit with adjustable time delays (typically 10-100 ms between rail ramp starts). Ramp rate: 0.1-1.0 V/ms to limit inrush current. Monitor all rails with voltage supervisors (threshold ±5% of nominal).
