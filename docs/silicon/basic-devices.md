...
2. **[Texturing](../glossary/texturing.html)** (optional, improves light trapping): Anisotropic etch in KOH/IPA solution (2-5% KOH, 5-10% IPA, 70-80°C, 20-30 min). Pyramidal texture on <100> surface reduces reflection from ~35% to ~10%.
...
**[Screen printing](../glossary/screen-printing.html)** (for solar cell contacts):
...
**[Sputtering](../glossary/sputtering.html)** (later, more versatile):
...
- **Reverse bias breakdown**: When reverse voltage exceeds a threshold, current increases dramatically. **[Avalanche breakdown](../glossary/avalanche-breakdown.html)** (dominant for breakdown voltages >7V): accelerated carriers collide with lattice atoms, ionizing them and creating more carriers (chain reaction). **[Zener breakdown](../glossary/zener-breakdown.html)** (dominant for breakdown voltages <5V): quantum mechanical tunneling of carriers through the narrow depletion region in heavily-doped junctions.
...
- **Operating regions**: **[Cutoff](../glossary/cutoff.html)** (base-emitter junction not forward biased: Ic ≈ 0). **[Active](../glossary/active.html)** (base-emitter forward biased, base-collector reverse biased: Ic = β·Ib, amplifier operation). **[Saturation](../glossary/saturation.html)** (both junctions forward biased: Vce ≈ 0.2V, switch "on" state). **[Reverse active](../glossary/reverse-active.html)** (junctions reversed, β very low, rarely used).
...
- **[Ball bonding](../glossary/ball-bonding.html)** (gold wire): Flame-off creates a small ball at the wire tip. Thermosonic bonding (heat + ultrasonic vibration + pressure) attaches the ball to the aluminum bond pad on the die. Wire is looped to the package lead, where a second bond (wedge) is made. Ball bonding is the dominant method for ICs and most discrete devices.
- **[Wedge bonding](../glossary/wedge-bonding.html)** (aluminum wire): Ultrasonic energy and pressure form a wedge bond at both die and lead. No heat required (can be done at room temperature). Used for power devices (thicker aluminum wire, 100-500 μm, carries higher current) and for devices sensitive to gold contamination.
