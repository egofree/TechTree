...
|---------|---------------|----------|
| Lot at AQL quality rejected (producer's risk α = 5% realized) | Statistical variation — even good lots (at AQL defect rate) are rejected 5% of the time by design; or sample size too small for the lot, inflating sampling error | Accept as inherent sampling risk; if rejection rate for good supplier exceeds 5%, verify AQL and inspection level match the contract; consider double sampling to give borderline lots a second chance |
| Switching rules stuck in tightened inspection for >10 consecutive lots | Supplier quality genuinely degraded — 2 of 5 consecutive lots rejected triggered tightened, and 5 consecutive accepts required to return to normal is not achieved | Work with supplier on corrective action; investigate incoming material quality trend; verify defect classification criteria are not overly strict; review whether the defect types causing rejection are critical or cosmetic |
| 100% wafer sort misses defective die that fail at customer site | Test coverage insufficient — probe test patterns do not exercise all functional paths; or test temperature (+25°C only) does not catch temperature-dependent failures exposed at −40°C or +125°C in the field | Add functional test patterns covering untested logic paths; implement multi-temperature test (at minimum −40°C, +25°C, +85°C); add burn-in screening (125–150°C, 24–168 hours) to accelerate infant mortality failures before shipment |
| Incoming photoresist passes inspection but causes pattern defects in production | Viscosity measurement at ±0.5 cSt not sensitive enough — viscosity drifted enough to change spin-coating thickness beyond process window; or shelf life exceeded but not caught by incoming test | Add coating thickness verification to incoming inspection (spin-coat test wafer, measure with ellipsometer); verify expiration date matches manufacturer CoA; store photoresist per manufacturer temperature requirements to prevent premature aging |
| Skip-lot sampling misses a bad lot — defective material reaches production | Supplier had a one-time quality excursion between skip inspections; skip-lot frequency (1 in 3 or 1 in 4) allowed the defective lot through uninspected | Immediately return to 100% lot inspection per skip-lot rules; investigate root cause with supplier; require 20 consecutive accepted lots before re-qualifying for skip-lot; review whether skip-lot is appropriate for this material's criticality |
| Double sampling first sample rejects a lot (d₁ > r₁) that single sampling would have accepted | Double sampling first sample size n₁ is smaller than single sampling n, with correspondingly tighter reject number — a lot with defects concentrated in the first sample appears worse than it is | This is inherent to double sampling — the smaller first sample has higher statistical variability; verify the reject number r₁ is correctly set per MIL-STD-105E tables for the lot size and AQL; do not override the statistical plan based on intuition |
| Variables sampling rejects lot that passes attribute inspection on same units | Variables plan detects mean shift toward spec limit even when individual units are within tolerance — more sensitive to process drift than pass/fail attribute inspection | The variables plan is correctly detecting a developing problem before it produces out-of-spec units; investigate the process mean shift; verify measurement system gage R&R is adequate — poor measurement repeatability inflates apparent variation |
| Human visual inspector misses 15–20% of defects on repetitive inspection task | Inspection fatigue — human detection rate drops after 20–30 minutes of continuous visual inspection; inadequate lighting or magnification for defect size | Rotate inspectors every 20–30 minutes; ensure lighting meets minimum 1,000 lux at inspection surface; provide 200–500× optical microscope for defect verification; implement automated optical inspection (AOI) for repetitive pattern checks |
| MIL-STD-105E sampling plan gives Ac = 0, Re = 1 — single defect rejects entire lot | Lot size and AQL combination yields the most stringent sampling plan (e.g., 500-unit lot, AQL 0.25%, Code H, sample 50 units); zero-defect acceptance is statistically harsh for moderate sample sizes | Accept the statistical plan — Ac = 0 maximizes consumer protection for semiconductor-grade materials; if producer's risk is too high, negotiate larger sample size or alternative AQL with supplier; for truly critical materials, zero-defect acceptance is appropriate |
| Destructive cross-section inspection results cannot be replicated on a second sample | Sample preparation variation — cleaving, polishing, and SEM imaging of cross-sections is operator-dependent; the specific location chosen for cross-section may not be representative of the full wafer | Standardize sample preparation procedure; specify exact measurement location (e.g., "center of die at coordinates X,Y"); take multiple cross-sections from different wafer locations and report range, not single point value |
