...
**Trade-offs**. The novolac/DNQ positive-tone system provides 0.25-1.0 μm resolution with i-line exposure and carries 50+ years of production understanding. SU-8 negative resist can pattern 5-500 μm thick films in a single coat, unmatched for MEMS and electroplating molds. The trade-offs are speed and resolution limits. Bitumen resist requires hours of exposure with ~100 μm+ resolution, suitable only for initial experimentation. Isoprene-based negative resists suffer swelling during development, limiting resolution to ~2-3 μm.
...
**Trade-offs**. E-beam mask writing at 10-50 nm spot size enables 4×/5× reticles with features far smaller than optical lithography limits. The pellicle membrane keeps particles 5-10 mm above the mask plane — out of depth of focus, so they do not print as defects. The penalties are cost and repair artifacts. E-beam write time of 1-8 hours per reticle makes masks expensive ($5K-50K each). FIB mask repair introduces Ga⁺ contamination and ≤10 nm substrate damage, leaving repaired sites with slightly different optical properties.
...
**Trade-offs**. Projection printing with NA 0.28-0.60 achieves sub-micron resolution via the Rayleigh criterion (k₁·λ/NA). Laser interferometer stage positioning with λ/1000 ≈ 0.6 nm resolution enables precise step-and-repeat exposure. The trade-offs are depth of focus and source degradation. Depth of focus shrinks as NA increases (DoF ≈ ±k₂·λ/NA²), requiring flatter wafers and better autofocus. Mercury arc lamps degrade ~10% per 100 hours and risk catastrophic envelope rupture releasing mercury.
...
**Trade-offs**. Oxygen plasma ashing removes bulk resist at 1-5 μm/min as volatile CO₂ + H₂O, producing no liquid waste stream. The standard ash → piranha → rinse sequence removes all resist types including heavily crosslinked post-implant crust. The hazards are severe. Piranha solution (H₂SO₄:H₂O₂ 3:1) reaches 100-130°C and can explode on contact with organics — the most dangerous wet chemical in the fab. Acetone cannot remove hard-baked or plasma-crosslinked resist, limiting it to non-critical stripping only.
...
**Trade-offs**. The DNQ dissolution inhibitor provides 100-1000× dissolution rate contrast between exposed and unexposed regions, giving sharp pattern definition. TMAH developer is metal-ion-free, eliminating the Na/K contamination that shifts MOS threshold voltage. The limitations are wavelength and infrastructure. Resolution is limited to 0.25-1.0 μm with i-line (365 nm) — shorter wavelengths require different resist chemistry. Novolac resin requires phenol-formaldehyde polymerization, creating an organic chemistry infrastructure dependency.
...
**Trade-offs**. SU-8 chemical amplification provides extreme sensitivity — one photon triggers 100-1000 cross-linking reactions — and it patterns 5-500 μm thick films in a single coat, unmatched for MEMS structures and electroplating molds. The limitations are swelling and stress. SU-8 swelling during PGMEA development distorts features below ~2 μm and degrades to 10-20 μm resolution for films >100 μm. Historical isoprene-based negative resists suffered xylene swelling that limited resolution to ~2-3 μm, which is why they were replaced by positive resists in the 1970s.
...
**Trade-offs**. Spin coating thickness follows t ∝ 1/√ω, predictable and repeatable from 0.5-5 μm by adjusting spin speed. Soft bake at 90-110°C for 60-90 seconds achieves ±1°C uniformity on a hot plate, sufficient for CD control. The complications are edge bead and parameter count. Edge bead requires a separate EBR solvent spray step, adding process complexity. And CD depends on 6+ simultaneous parameters (dose, focus, resist thickness, development time, PEB temperature, reticle CD), demanding complex statistical process control.
...
**Trade-offs**. The fused silica mask substrate is polished to λ/4 flatness with Cr optical density >3.0, blocking >99.9% of exposure light. E-beam placement accuracy of ±20 nm over a 100×100 mm pattern area enables 4× reticles for sub-micron wafer features. The bottlenecks are write time and repair damage. E-beam write time of 1-8 hours per reticle makes masks expensive and creates a throughput bottleneck. FIB Ga⁺ repair introduces ion contamination and ≤10 nm substrate damage at repaired sites.
...
**Trade-offs**. The focus-exposure matrix (FEM) identifies the process "sweet spot" where CD is most tolerant of dose and focus variations. SPC with X-bar and R charts catches CD drift before it impacts production yield. The challenges are tight focus budgets and complex error allocation. DoF of only ±1.0 μm for i-line at NA 0.50 means any wafer topography or tilt degrades CD uniformity. And the CD budget of 100 nm (3σ) for 1.0 μm features must be allocated across 6+ error sources, leaving thin margins.
...
**Trade-offs**. The yield model (1 - D·A)ⁿ provides a clear quantitative link between defect density and profitability, driving investment in contamination control. Reducing defect density from 0.5 to 0.1/cm² per layer improves yield from 13% to 70% for a 7-layer process — an enormous economic incentive. The challenge is accumulation. The exponential yield model means each additional lithography layer multiplies yield loss, making 7 critical layers already challenging. And defect density is cumulative across all process steps — contamination from any single step degrades overall yield.
...
**Trade-offs**. Laser interferometer stage positioning at λ/4 ≈ 158 nm resolution provides repeatable field-to-field placement. The nitrogen-purged lens barrel at ±0.1°C prevents contamination and refractive index drift in $100K-$1M projection optics. The maintenance burdens are lamp replacement and lens risk. Mercury lamp replacement every 800-1200 hours requires 30+ minute cooldown, lamp alignment, and intensity recalibration. Lens cleaning risks damaging $100K-$1M optics — a single-wipe-only protocol with no rubbing allowed.
...
**Trade-offs**. Grid correction (translation, rotation, scaling) from calibration wafer measurements reduces systematic overlay errors to ±30-50 nm residual. OPC on reticles compensates for proximity effect, ensuring dense and isolated features print at the same CD. The challenges are mark degradation and shrinking windows. Alignment marks degrade with each process step (oxidation, etch, deposition), so they must be designed 5-10× larger than minimum features. And the common process window shrinks as feature sizes decrease, demanding tighter tolerance on all parameters simultaneously.

## Photoresist Stripper Chemistry

**Resist removal after high-dose implant or plasma etch**:
- Heavily crosslinked resist (from ion implantation at >10¹⁵ ions/cm², or from extended plasma etching with polymerizing chemistries) becomes a carbonaceous glass. Standard solvents cannot dissolve it. Oxygen plasma ashing removes the bulk, but a thin carbon-rich crust remains.
...
**Trade-offs**. Piranha (H₂SO₄:H₂O₂ 3:1) generates persulfuric acid that attacks even heavily crosslinked carbon crust — the most aggressive organic removal available. Downstream plasma ash removes bulk resist without ion bombardment damage to underlying films. The complications are plumbing and disposal. Piranha attacks most organic drain materials, requiring dedicated acid-resistant plumbing and fume hood. And piranha must be decomposed by slow dilution with cold water before disposal — it cannot be stored.
...
**Trade-offs**. Chemical amplification provides 10-50 mJ/cm² sensitivity versus 100-200 mJ/cm² for DNQ-novolac — 4-10× faster exposure. CAR contrast (γ = 5-15) is much higher than DNQ-novolac (γ = 2-4), producing steeper sidewalls for better pattern transfer. The vulnerabilities are airborne contamination and formulation trade-offs. Ppb-level airborne amines neutralize photogenerated acid, causing T-topping, which requires chemical filtration in cleanroom air. And PAG loading trades sensitivity against etch resistance — higher PAG reduces plasma resistance.
...
**Trade-offs**. BARC eliminates standing wave interference and swing curves, dramatically improving CD uniformity across varying substrate reflectivities. The immersion top coat prevents PAG leaching into water, protecting both resist performance and lens contamination. The costs are added steps and formulation difficulty. BARC adds an extra spin-coat step and must be removed during pattern transfer etch, adding process complexity. The top coat must balance hydrophobicity (70-90° contact angle) with developer strippability — a narrow formulation window.
...
**Trade-offs**. The pellicle keeps particles 5-10 mm above the mask plane — out of depth of focus, so they do not print as repeating defects on every die. Its >99% transmission at the exposure wavelength means no measurable light loss in the imaging path. The limitations are material and lifetime. Nitride pellicles absorb at DUV wavelengths, requiring fluoropolymer alternatives and an additional material supply chain. And the rated lifetime of ~10,000 cycles means pellicles are consumables requiring regular inspection and replacement.
...
**Trade-offs**. Actinic inspection at the exposure wavelength catches defects that would print but are invisible at other wavelengths. FIB repair achieves 10 nm precision with ≤10 nm placement error, making otherwise scrap masks usable. The trade-offs are speed and residual variation. Actinic inspection at 1-5 cm²/min is slow — a full reticle inspection takes 30-60 minutes. And repaired sites have slightly different optical density than original chrome, causing minor but measurable CD variation.
## Troubleshooting
