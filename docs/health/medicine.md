...
**Vital signs**:
- **Pulse**: Rate (beats/minute), rhythm (regular/irregular), strength (thready/bounding). Normal resting: 60-80 bpm. Fast (>100) = fever, blood loss, shock. Slow (<50) = hypothermia, certain poisonings.
- **Temperature**: Mercury thermometer: 35-42°C range, 0.1°C graduations. Normal: 36.5-37.5°C (oral). >38°C = fever. >40°C = hyperthermia (cool immediately). <35°C = hypothermia (warm gradually).
- **Urine analysis**: Color (dark = dehydration, red = blood, cloudy = infection). Sweet-smelling urine = diabetes (test with Benedict's solution).

**Triage categories**:
- **Immediate** (red): Airway obstruction, massive hemorrhage, severe burns (>20% BSA), shock. Treat within minutes.
...
- **Expectant** (black): Injuries incompatible with survival given available resources. Comfort measures only.

## See Also

- [Emergency Care](emergency-care.md) — triage, hemorrhage control, burns, fractures, dental emergencies
- [Infectious Disease Management](infectious-disease.md) — antisepsis, sterilization, disease treatment
- [Surgery Basics](surgery-basics.md) — wound management, surgical techniques, anesthesia
- [Pharmacology](pharmacology.md) — drug extraction, preparation, dosing, quality control
- [Diagnostics](diagnostics.md) — physical examination, vital signs, laboratory testing
- [Medical Instruments](medical-instruments.md) — instrument fabrication
