...
**Library of Congress Classification** (developed 1897-1901):
...
**Purpose-built technical classification** (for bootstrap context):
...
**Deacidification** (for acidic paper, pH below 6):
...
**Digitization workflow** (once semiconductor capability exists):
...
- **Closed stacks** (staff retrieves books): Maximum preservation — no public access to shelves, reduced theft risk, controlled environment. Slower access (request → retrieve → read → return). Preferred for rare and archival materials.
- **Open stacks** (browsing): Users access shelves directly. Faster access, encourages discovery. Requires wider aisles, more robust shelving, security measures (exit checks). Preferred for general circulating collections.
...
**Archival organization** (original order principle):
...
**Rebacking** (restoring a deteriorated spine):
...
|---------|---------------|----------|
| Mold outbreak on stored volumes despite climate control | Relative humidity exceeded 65% RH threshold — spores activated in an area with inadequate air circulation (below 2-4 air changes/hour) | Isolate affected items immediately; dry thoroughly; brush spores outdoors while wearing N95-equivalent mask; wipe shelves with 70% ethanol; install additional hygrometers and add silica gel desiccant canisters (regenerate at 120°C for 2 hours when indicator beads turn pink) |
| Acidic groundwood paper testing at pH 4.5 is already brittle (below 2 MIT folds) | Deacidification was delayed too long — once folding endurance drops below 2 MIT folds, alkaline treatment cannot restore mechanical strength | Photograph or microfilm brittle items on 35 mm silver halide film immediately before they disintegrate; do not waste magnesium bicarbonate on irrecoverable stock — prioritize items still above 20 MIT folds |
| Steel shelving causes condensation damage to books on exterior walls | Steel conducts heat faster than wood — cold exterior wall temperature drops shelf surface below dew point, condensing moisture onto book covers | Move steel shelving away from exterior walls minimum 300 mm; add thermal break (insulation board) between shelf uprights and wall; alternatively replace with oak shelving (20-25 mm thick boards, coated with linseed oil to resist moisture) |
| Card catalog author and subject entries show duplicate headings for the same concept | No master authority file of approved subject headings was maintained — catalogers used variant terms ("iron smelting" vs. "iron production") | Create a controlled vocabulary thesaurus where each concept gets one preferred term; cross-reference synonyms on "see also" cards; retrospectively reconcile duplicate cards and update shelflist |
| Wet books from a roof leak cannot be frozen within 48 hours | Insufficient freezer capacity or disaster supplies were stored inside the library building (also affected by the leak) | Air-dry with fans and interleaved blotting paper as fallback (accepts more distortion than freeze-drying); pre-position disaster recovery supplies (blotting paper rolls, plastic sheeting, portable generators) in a separate building per preparedness protocol |
| Daily hygrothermograph readings swing ±5°C over a 24-hour period | Thick masonry walls (≥400 mm) are absent or compromised; passive thermal mass insufficient to damp temperature fluctuations | Add interior insulation or secondary masonry layer to approach 400 mm thermal mass; if electricity is available, install thermostatically controlled heating (electric resistance or steam radiators) to maintain 18-22°C within ±2°C |
| Microfilm reader produces dim, unfocused images | Ground glass lens has accumulated dust or the projection lamp has shifted from its mount | Clean lens with lint-free cloth and ethanol; re-seat incandescent lamp in its housing at the correct focal distance; if lens is etched from age, re-grind or replace — optical quality directly affects readability at 15:1 reduction ratio |
| Ozone treatment for smoke-damaged books weakens paper | Ozone concentration exceeded 2.0 ppm or exposure exceeded 48 hours — ozone oxidizes odor compounds but also degrades cellulose | Limit ozone treatment to 0.5-2.0 ppm for maximum 24-48 hours; test paper fold endurance before and after; if paper is already fragile, use activated charcoal in sealed containers for odor absorption instead (slower but non-destructive) |
| Bound periodical volumes are falling apart within 5 years | Original paper covers were not removed before binding; issues sewn through fold but adhesive on covers caused acidic migration | Re-bind properly: remove all original paper covers, collate issues in order, sew through the fold with linen thread, bind in cloth or buckram with journal title, volume number, and year stamped on spine |
| Library turnover rate below 0.3 — collection not meeting user needs | Acquisition priorities misaligned with actual demand — circulation data not being used to guide purchases | Analyze circulation data by subject class to identify high-demand/low-holdings gaps; target a healthy turnover rate of 0.5-2.0; increase acquisitions in high-use classes and consider interlibrary loan for low-demand titles (fill rate target 85-95%) |
