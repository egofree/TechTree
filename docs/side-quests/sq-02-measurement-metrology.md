# SQ 2: Measurement, Timekeeping &amp; Metrology

**Starts**: Phase 1-3
**Priority**: Critical — you cannot build what you cannot measure.

## Problem

Precision manufacturing (Phase 3) and all semiconductor work requires consistent, calibrated measurement. Without standardized units and measuring instruments, interchangeable parts and repeatable processes are impossible.
...
**Length standard**:
- Define the base unit of length as a physical artifact: a bar of low-expansion alloy (invar — 64% Fe, 36% Ni, thermal expansion coefficient ~1.2 × 10⁻⁶/°C, Phase 5) or, before invar, well-annealed steel bar stored at constant temperature.
...
- Subdivide by precision dividing engine (Phase 3): Lead screw of known pitch rotates → carriage moves linearly → scribe marks at known intervals. Calibrate lead screw error by comparing with standard. Apply corrections.
...
- **Thermocouple** (Phase 4+): Two dissimilar metals joined at measurement junction. Voltage generated proportional to temperature difference between measurement junction and reference junction. Type K (chromel/alumel): -200°C to +1300°C, ~41 μV/°C. Type S (Pt/Pt-10%Rh): 0°C to +1600°C, ~6 μV/°C. Read with precision millivoltmeter or potentiometer. Accuracy ±1-2°C with reference junction compensation.
...
- **Construction**: Brass gears (cut on dividing engine from Phase 3). Steel pivots in brass or jeweled bearings (agate or synthetic ruby jewels reduce friction). Steel or Invar pendulum rod (Invar minimizes thermal expansion effect on period — length change alters period). Pendulum bob: heavy brass or lead lens, adjustable rating nut for fine time adjustment (turn nut → raise/lower bob → change effective length → change rate).
...
- **Steel rule**: Hardened and tempered spring steel, photo-etched or engine-divided graduations. Length 150-1000 mm. Graduated in mm and 0.5 mm. Flatness ≤0.02 mm over 300 mm. Made by surface grinding against lapped surface plate (from Phase 3 iterative metrology bootstrap).
...
- **Alternative (Phase 5+)**: Zener diode reference (temperature-compensated Zener, 6.95V, stability ~1-10 ppm/year in oven-controlled enclosure). Solid-state, more robust than standard cell.
...
- **Length measurement**: Laser interferometer (Phase 8+) measures distance by counting wavelengths of laser light. Resolution: λ/4 ≈ 158 nm (HeNe laser, λ = 633 nm). With interpolation: 1-10 nm resolution. THE most precise length measurement method available.

## Integration Points

| Phase | Contribution |
|-------|-------------|
| Phase 1 | Basic length (rulers), volume (containers), weight (balance), time (water clocks) |
| Phase 3 | Precision instruments (micrometers, calipers, gauge blocks), thread standards, pendulum clocks |
| Phase 4 | Electrical measurement standards (voltmeters, ammeters, bridges), electrical clocks |
| Phase 6 | Optical metrology, interferometry, spectrometers |
| Phase 7 | Crystal orientation measurement, wafer thickness/flatness, quartz frequency standards |
| Phase 8 | Feature size measurement (microscopy), overlay registration, defect inspection, laser interferometry |
