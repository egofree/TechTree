...
|---------|---------------|----------|
| Pareto analysis shows no single defect type dominating (>10 types each at 5–8%) | Defect taxonomy too granular — categories split one physical defect type into multiple classification labels; or genuine multi-source yield loss from unstable process | Consolidate defect categories — group by root mechanism (particle, pattern, contamination) rather than symptom; re-run Pareto with merged categories to reveal the true vital few |
| Fishbone diagram generates 50+ potential causes with no clear priority | Brainstorming session lacked data-driven constraint — participants listed hypothetical causes without referencing SPC charts or tool maintenance logs | Cross-reference each hypothesized cause against available data (FDC traces, maintenance records, SPC trends); circle only causes with supporting evidence; eliminate causes contradicted by data before investigating |
| FMEA RPN scores are uniformly high (all >200) — no useful prioritization | Severity ratings inflated (everything rated 8–10); or occurrence ratings set conservatively for unknown failure modes instead of using historical defect rate data | Recalibrate severity scale: reserve 8–10 for truly catastrophic/circuit-killing defects; use actual production defect rate data for occurrence ratings (1 in 10⁶ = rating 1; >1 in 10 = rating 10); focus on highest RPN items first |
| Murphy yield model predicts 85% yield but actual production yields 60% | Model assumes random spatial defect distribution — real defects cluster (single tool contamination, handling damage belt at wafer edge); D₀ measurement from test wafers underestimates production defect density | Measure D₀ from production wafers, not test wafers; switch to Seeds model Y = 1/(1+D₀A) with clustering parameter α = 1–2 for more conservative (realistic) prediction; account for systematic yield loss mechanisms separately from random defect model |
| 5 Whys analysis stops at "operator error" without reaching systemic root cause | Analysis stopped too early — "operator error" is a symptom of inadequate training, unclear procedures, or poor human-factors design, not a root cause | Continue asking "why" beyond operator error: Why did the operator make the error? → Procedure ambiguous → Why ambiguous? → Written by engineer without production floor input → Systemic root: all procedures must be validated by production operators before release |
| Commonality analysis finds defective lots correlate with one chamber but no parameter is out of spec | Chamber condition drift below individual parameter alarm thresholds — multiple parameters each drifting slightly (etch rate +3%, uniformity +2%, pressure +1%) combine to produce defects that no single parameter alarm catches | Deploy multivariate FDC (PCA/PLS model) on the affected chamber — multivariate T² statistic detects correlated drift that individual univariate limits miss; retrain model with 50–100 baseline runs after chamber clean |
| Yield learning curve plateaus — no improvement for 3+ months despite engineering effort | Engineering team chasing low-impact defects (bottom of Pareto) while a new, unrecognized defect type has emerged as the yield limiter; or top defect root cause is fundamental (equipment capability limit) not improvable without capital investment | Re-run Pareto analysis with current data — the yield limiter may have shifted; investigate whether current top defect requires equipment upgrade (e.g., new inspection tool) rather than process optimization; consider DOE to explore wider process window |
| Composite yield calculation shows 60.6% for 500 steps at 99.9% per-step — actual yield is 45% | Per-step yield estimates are optimistic; some steps run at 99.7–99.8% not 99.9%; or defect interaction effects (non-random) reduce actual yield below the multiplicative model | Measure actual per-step defect density for each process step; recalculate composite yield with measured (not estimated) values; identify which specific steps contribute most to yield loss using defect budget analysis; target Y_target^(1/N) per-step yield requirement |
| Stratified Pareto shows defect only occurs on Tool Etch-12, Chamber 3 — but Chamber 3 passes all monitor wafers | Monitor wafer test conditions differ from production conditions (different recipe, different wafer type, different run length); monitor wafer is not sensitive to the specific failure mode | Run production-qualification wafers (same recipe, same wafer type) on Chamber 3 instead of standard monitor recipe; add defect-type-specific monitor tests; consider dedicating Chamber 3 to non-critical layers until root cause is found |
| Root cause identified and corrective action implemented — but defect recurs 2 months later | Corrective action addressed symptom, not root cause; or corrective action was a temporary fix that degraded over time; or the defect has multiple independent root causes | Re-run full RCA (Kepner-Tregoe) — the recurrence may be a different root cause producing the same symptom; verify the original corrective action is still in place and effective; check for new process changes that may have reintroduced the failure mode |
