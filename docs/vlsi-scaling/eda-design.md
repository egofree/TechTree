...
> **Dependencies**: [`chemistry.dopant-etch-gases`](../chemistry/dopant-etch-gases.md), `computing`,
...
|---------|---------------|----------|
| STA shows setup violations on multiple paths after synthesis | Wire load model underestimates interconnect delay at sub-130 nm; physical synthesis not run with actual placement data | Run physical synthesis with placed wire loads instead of statistical models; upsize cells on failing paths or add pipeline stages; re-synthesize with tighter timing constraints |
| DRC reports >1000 antenna rule violations on metal layers | Long metal wires connected to gate electrodes accumulate >100-400× charge ratio during plasma etching; no jumper vias to upper layers inserted | Run automatic antenna fixing: insert jumper vias breaking long wires at upper metal layers; add antenna diodes on violated nets; verify fix does not introduce new density rule violations |
| LVS mismatches — netlist comparison reports shorts between power domains | Missing via array or misaligned via in power grid causing adjacent nets to merge in extraction; global routing congestion caused via omission | Trace LVS error coordinates to layout; inspect via coverage at reported locations; add missing vias and re-run LVS; check power grid connectivity separately before full-chip LVS |
| IR drop exceeds 5-10% of Vdd at chip center — timing failures in silicon | Power grid metal straps too narrow in high-activity regions; simultaneous switching current underestimated in power analysis | Widen power grid straps in IR drop hotspots; add decoupling capacitors in whitespace near failing blocks; re-run power grid FEA analysis with accurate activity factors; target 10% margin on supply voltage |
| Formal equivalence checking fails between RTL and gate-level netlist | Synthesis optimization restructured logic (retiming, constant propagation, or state encoding change) creating functionally equivalent but structurally different netlist | Review equivalence checker diagnostics for don't-care optimization; verify retimed registers match timing constraints; if false mismatch, update formal constraints and re-verify |
| Crosstalk adds >50% delay variation on critical nets at sub-130 nm | Victim-aggressor coupling capacitance exceeds budget; insufficient spacing or missing ground shield between long parallel runs | Add shielding wires (ground lines between critical signal pairs); increase spacing on high-coupling nets via wire spreading; re-run SI analysis to verify noise pulses stay below receiving gate switching threshold |
| Electromigration signoff fails on power bus — MTTF below 10-year target | Current density exceeds 1 mA/μm² for copper at 105°C; power wires too narrow for worst-case switching current in high-activity blocks | Widen failing wire segments per Black's equation requirements; verify current profiles use worst-case activity factors; consider migrating high-current paths to upper thick metal layers (M9+ at 1-4 μm pitch) |
| Full-chip OPC runtime exceeds 72 hours per layer | 10⁸-10⁹ shapes per layer with iterative lithography simulation; insufficient compute cluster parallelism | Allocate 100-1000 CPU cluster with GPU-accelerated lithography simulation (10-50× speedup); partition full-chip into tiles for parallel OPC; prioritize critical layers for model-based OPC, use simpler rule-based OPC on non-critical layers |
