...
- **Decoction**: Boil coarse plant material in water for 15-30 minutes. Strain. Suitable for roots, bark, seeds (tough cell walls break down with heat). Use within 24 hours — water extracts spoil rapidly.
- **Percolation**: Pack ground plant material in a tall vessel. Slowly drip solvent through from top. Collect percolate at bottom. More efficient than maceration — continuous fresh solvent contact. Produces concentrated extract.
- **Steam distillation**: Pass steam through plant material. Volatile oils vaporize and condense with the steam. Separate oil layer (lighter than water, floats). Used for essential oils (eugenol from cloves, menthol from peppermint, thymol from thyme). Yields typically 0.5-3% by weight.

**Solvent selection**:
- **Water**: Polar solvent. Extracts sugars, glycosides, tannins, some alkaloids (as salts). Cheapest, safest. Spoilage risk — use promptly or add ethanol as preservative (≥15% ethanol by volume).
...
**Tinctures** (ethanol-based extracts):
...
**Powders**:
- **Trituration**: Grind dried plant material or purified drug in mortar with pestle to fine powder (pass through fine sieve). **Geometric dilution**: For potent drugs, mix active with equal weight of inert diluent (lactose, starch), then repeat doubling until uniform. Prevents dangerous dose variation.

**Pills and tablets**:
- Mix powdered active ingredient with binder (honey, gum arabic, starch paste, 5-10% by weight). Roll into uniform spheres or compress in a die. Coat with sugar or shellac to mask taste and control dissolution. Each pill must contain identical dose — weigh individually during production setup, then check batch average periodically.

**Ointments and salves**:
- Melt beeswax (10-20%) with oil base (olive, lard, petroleum jelly) at 60-70°C. Stir in powdered drug or oil extract. Pour into containers, let cool and solidify. Typical consistency: 1:4 to 1:10 drug:base ratio. For water-miscible creams, add borax as emulsifier to combine oil and water phases.
...
**Analgesics** (see medicine.md for clinical use):
...
**Antiseptics** (see medicine.md for clinical application):
...
- **Injection** (advanced — requires glass syringes + needles): Intravenous (immediate onset), intramuscular (sustained release). Sterile technique mandatory. Hypodermic needle: drawn-glass syringe + steel needle (requires precision metalworking).
...
- **Percolation**: Pack coarsely ground plant material in a conical percolator. Moisten with a small amount of solvent and let swell for 4-6 hours. Then slowly add solvent from the top at a controlled rate (1-3 mL/min per kg of plant material). Collect percolate at the bottom. Continue until the percolate tests negative for active compound (specific chemical test or taste/color indicator). More efficient than maceration: continuous fresh solvent contacts plant material, maintaining the concentration gradient that drives extraction.
- **Decoction**: Boil coarse plant material (roots, bark, woody stems) in water for 15-30 minutes at 100°C. Cool, strain. The high temperature breaks down tough cell walls that cold extraction cannot penetrate. Use within 24 hours — no preservative. Ratio: typically 1:10 to 1:20 (plant weight:water volume). Suitable for: cinchona bark (quinine), willow bark (salicin), ephedra (ephedrine).

**Purification by crystallization**:
- **Principle**: Dissolve crude extract in hot solvent. Cool slowly. Pure compound crystallizes first (higher concentration, lower solubility at low temperature). Impurities remain in solution. Filter crystals. Repeat recrystallization for higher purity.
- **Example: salicylic acid from willow bark**: Macerate willow bark in ethanol for 3 days. Filter. Evaporate ethanol to obtain crude extract. Dissolve crude in hot water. Add NaOH to convert salicylic acid to sodium salicylate (water-soluble). Filter to remove insoluble impurities. Acidify filtrate with H₂SO₄ to precipitate salicylic acid. Collect by filtration. Recrystallize from hot water. Pure salicylic acid forms white needles, melting point 159°C (sharp melting point confirms purity — impure substances melt over a wide range).
- **Solvent choice**: Choose a solvent where the target compound is very soluble when hot but poorly soluble when cold. Water works for many organic acids and salts. Ethanol for less polar compounds. Mixed solvents (water-ethanol) for intermediate polarity.

**Dosage form manufacturing**:
- **Powder preparation**: Dry plant material or purified drug at 40-60°C until constant weight. Grind in mortar with pestle, then through a series of sieves (coarse to fine). Target particle size <150 μm (passes through a 100-mesh sieve). Finer particles dissolve faster and absorb more consistently. For potent drugs, use geometric dilution: mix active with equal weight of inert diluent (lactose, starch), then double the mixture with more diluent, repeating until the active is uniformly distributed at the target concentration. This prevents dose variation from poor mixing.
- **Tablet compression**: Mix active ingredient with binder (starch paste, 5-10% by weight), filler (lactose or calcium phosphate), and lubricant (talc or magnesium stearate, 1-2%). Compress in a steel die at 5-20 kN using a single-punch or rotary tablet press. Target hardness: 4-8 kgf (too soft → crumbles in handling; too hard → does not disintegrate in stomach). Disintegration time: <15 minutes in water at 37°C (test by dropping tablet into warm water and timing).
- **Tincture preparation**: Combine plant material with 45-70% ethanol at 1:5 or 1:10 ratio (plant weight:solvent volume). Seal in glass container. Shake daily for 14-28 days. Decant and press residue. Filter through paper or cloth. The ethanol preserves the preparation for years. Standardize by comparing color, odor, and specific chemical tests to a reference batch.
...
- **Oxidation**: Oxygen attacks susceptible functional groups (phenols, unsaturated bonds, thiols). Vitamins A, C, and E are oxidation-prone. Adrenaline (epinephrine) turns brown when oxidized, losing potency. Prevent by adding antioxidants (ascorbic acid, sodium metabisulfite) and storing in sealed, air-free containers.
- **Photolysis**: Light energy breaks chemical bonds. Riboflavin (vitamin B₂) degrades under UV light in hours. Nitrates and nitrites form carcinogenic nitrosamines under UV exposure. Store light-sensitive drugs in amber glass bottles or opaque containers.
- **Temperature effects**: Reaction rate approximately doubles for every 10°C temperature increase (Arrhenius rule). Refrigeration at 2-8°C extends shelf life 4-10× compared to room temperature storage. Freezing may damage some formulations (emulsions separate, solutions precipitate).

**Stability testing protocol**:
- **Long-term**: Store at 25±2°C, 60±5% RH. Test at 0, 3, 6, 9, 12, 18, 24, and 36 months. Establish actual shelf life under real conditions.
...
**Capsule preparation** (requires gelatin):
...
**Sterile preparation** (for injectable drugs):
...
- **Ether + epinephrine**: Epinephrine sensitizes the heart to catecholamine-induced arrhythmias under ether anesthesia. Do not give epinephrine to patients under ether anesthesia (risk of ventricular fibrillation).
- **Morphine + other CNS depressants**: Additive respiratory depression when morphine is combined with alcohol, ether, barbiturates, or other opioids. Dose reduction of both agents is mandatory when combined.
- **Tetracycline + calcium/iron/dairy**: Divalent and trivalent cations (Ca²⁺, Fe²⁺/³⁺, Mg²⁺, Al³⁺) chelate tetracycline in the gut, preventing absorption. Take tetracycline on an empty stomach, 2 hours before or after calcium/iron supplements or dairy products.

**Pregnancy precautions**:
- Many drugs cross the placenta and affect the fetus. First trimester (organogenesis, weeks 3-12) is the period of highest teratogenic risk. Avoid all non-essential drugs during first trimester.
...
- **Dispensing balance**: Sensitivity 10-100 mg. Sufficient for most tablet and powder compounding. Pan balance with sliding weight or dial. Check zero before each weighing.
- **Volume measurement**: Graduated cylinders (10-1000 mL) for liquids. Pipettes (1-25 mL, graduated or volumetric) for precise liquid transfer. Volumetric flasks (25-1000 mL) for preparing solutions of exact concentration. All glassware should be Class B tolerance or better. Calibration: mark volumetric flasks at the meniscus line.
...
- **Common name**: White willow, meadowsweet, foxglove, cinchona. Useful for communication with non-specialists but potentially confusing (different plants may share the same common name in different regions).
- **Active constituent**: The specific chemical responsible for the drug's therapeutic effect (e.g., salicin in willow bark, morphine in opium, quinine in cinchona bark). Isolating and identifying the active constituent is the goal of pharmaceutical chemistry.
- **Pharmacopoeia standards**: A pharmacopoeia is an official reference book listing drug standards (identity, purity, potency, dosage). Historical examples: De Materia Medica (Dioscorides, ~70 AD), London Pharmacopoeia (1618), United States Pharmacopeia (USP, 1820). A bootstrap pharmacopoeia should document: plant source (with botanical drawing), harvest time and method, preparation method, dosage range, known side effects, and contraindications.

**Record keeping for drug production**:
...
- **Procurement**: Source raw materials from known, trusted suppliers. Test each incoming batch (identity test at minimum: appearance, odor, and one chemical test). Reject any material that fails testing.
- **Storage chain**: Maintain cold chain for temperature-sensitive drugs (insulin, vaccines, some antibiotics). Monitor refrigerator temperature daily (2-8°C). Discard any drug that has been frozen or overheated.
...
- **Visualization**: Remove plate when solvent front reaches ~1 cm from the top. Mark the solvent front. Dry plate. Visualize under UV light (254 nm and 366 nm) or spray with detection reagent (Dragendorff's reagent for alkaloids → orange spots; ninhydrin for amino acids → purple; ferric chloride for phenolics → blue/black). Compare Rf values (distance traveled by compound ÷ distance traveled by solvent front) between sample and standard. Matching Rf confirms identity.
- **Interpretation**: A compound in the test sample that matches the Rf and color of the reference standard is tentatively identified. Additional spots indicate impurities or other compounds. Rf values shift with temperature, humidity, and solvent batch, so always run reference standards on the same plate for valid comparison.

**Microbial and heavy metal quality limits**:
