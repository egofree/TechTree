# SQ 4: Mechanical Computing &amp; Automation

**Starts**: Phase 3
**Priority**: Medium-High — dramatically accelerates design calculations and process control.

## Problem

Design and process control calculations for semiconductor work are extremely tedious by hand. Electronic computers arrive very late (Phase 8). Mechanical computing bridges the gap, enabling faster iteration and more complex designs.
...
- **Production**: Compute by difference methods (add small increments using known derivatives). Typeset and print via printing press (SQ 1). Multiple proofreaders verify against independent calculations. Errors in tables cause engineering failures — accuracy is paramount.
...
- **Construction requirements**: Precision gear-cutting, milling, drilling, and finishing — Phase 3 machine shop capability essential. Each calculator contains 1000-3000 precision parts. Manufacturing time: 100-300 skilled labor-hours per unit.
...
- **Manufacturing challenge**: Requires thousands of parts with tolerances of ±0.025 mm. Precision casting, gear-cutting, milling, grinding. Phase 3 machine tools are sufficient. Assembly and adjustment by skilled mechanic over weeks-months.
...
- **Application**: Census data, inventory tracking, payroll, process recipes (encode process parameters on cards → feed into machine that controls cam timing), and later (Phase 8) as program input for early computers.

## Integration Points

| Phase | Contribution |
|-------|-------------|
| Phase 3 | Precision gears for calculators; jigs/fixtures for production; slide rules |
| Phase 4 | Governors for steam engines; process control instruments; punch card systems |
| Phase 5 | Calculations for chemical processes, distillation column design |
| Phase 7 | Crystal growth process control, temperature profiling |
| Phase 8 | Stepper motor control, process sequencing, mask pattern calculation, early computers |
