...
|---------|---------------|----------|
| Chamber fails to reach base pressure of 5×10⁻⁸ Torr after 24-hour bakeout | Leak at flange seal; outgassing from cold spot during bakeout; H₂ accumulation from turbo pump exhaust back-diffusion | Helium MSLD leak check all CF flanges (target <10⁻⁹ Torr·L/s for sputtering); verify bakeout blanket coverage eliminates cold spots (±10-20°C uniformity); add titanium sublimation pump for H₂ gettering |
| Turbomolecular pump H₂ compression ratio only 10²-10⁴ — chamber hydrogen partial pressure rising | Light gases poorly pumped by blade stages; H₂ thermal velocity (~1300 m/s) exceeds blade tip speed (150-350 m/s) | Add supplementary pumping: cryopump with charcoal cryosorbent at 10-20 K, or titanium sublimation pump for chemical H₂ gettering; verify backing pump has adequate H₂ throughput |
| Ion pump fails to start — arcing at pressure >10⁻³ Torr | Roughing pump did not bring chamber below 10⁻³ Torr before ion pump activation; excessive gas load overheating cathode | Verify roughing pump brings chamber to <10⁻³ Torr before switching to ion pump; check for virtual leaks (trapped volumes under screws); use turbo pump for initial pump-down, then switch to ion pump |
| Cryopump regeneration takes >4 hours — production chamber unavailable too long | Cryopanels saturated with frozen gases (>10⁶ Torr·L capacity); roughing pump too slow for pump-out phase | Schedule regeneration during planned maintenance; verify roughing pump speed for pump-out phase; consider larger cryopump or dual-pump installation with staggered regeneration to maintain tool availability |
| Capacitance manometer reading drifts >0.5% per day | Gauge head temperature not controlled to 40-50°C; diaphragm contaminated with process deposits; zero offset drift | Verify gauge temperature control; zero the gauge at operating pressure; clean or replace diaphragm head if contaminated; perform annual calibration against spinning rotor gauge transfer standard |
| Diffusion pump oil backstreaming detected on wafer — contamination particles in film | Foreline pressure exceeds 0.5 Torr; cold cap refrigeration failed; chevron baffles saturated | Maintain backing pressure <0.5 Torr with operational roughing pump; verify cold cap cooling; clean or replace chevron baffles; add liquid-nitrogen cold trap (77 K) for near-100% vapor interception |
| Sputtering process pressure unstable — ±5% oscillation at 2-10 mTorr Ar | Throttle valve PID tuning inadequate; capacitance manometer feedback loop response >1 second; gas flow pulsation from MFC | Retune throttle valve PID for <1 second response and ±0.5% stability; verify capacitance manometer signal is noise-free; replace MFC if flow pulsation detected; check for air leaks in gas delivery lines |
| Viton O-ring sealed system cannot reach below 10⁻⁸ Torr | Viton outgassing at ~10⁻⁸ Torr·L/(s·cm²); helium permeation through elastomer at ~10⁻⁹ Torr·L/(s·cm) | Replace Viton O-rings with CF copper gasket seals for UHV (<10⁻⁹ Torr); if elastomer required, bake Viton at 200°C max for 24 hours to reduce outgassing; consider using PTFE seals with lower outgassing |
