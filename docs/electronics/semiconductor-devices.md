...
|---------|---------------|----------|
| BJT β much lower than expected | Base width too wide (insufficient diffusion drive-in), or emitter doping too low | Increase drive-in temperature (1100-1150°C range) or time (extend 30-60 min); verify furnace temperature uniformity (±0.5°C across load zone, measured with calibrated thermocouple); check POCl₃ source temperature (bubbler at 20°C ±1°C for consistent deposition rate) |
| MOSFET Vth too high | Gate oxide too thick (exceeds 100 nm for 5 V target), or fixed oxide charge too high (>10¹¹ cm⁻² from contamination) | Reduce oxidation time/temperature (dry O₂ at 900°C gives ~15 nm/hour growth rate for thin oxides); improve pre-oxidation clean (RCA clean: NH₄OH:H₂O₂:H₂O at 75°C for 10 min removes organics, HCl:H₂O₂:H₂O removes metals); check for sodium contamination (Na⁺ mobile ions shift Vth by 0.1-1.0 V per 10¹⁰ cm⁻²) |
| MOSFET gate leakage | Gate oxide pinhole defect from contamination or particulate fallout (>1 per cm² in Class 1000 cleanroom) | Improve cleanroom practice (maintain Class 1000 or better, garments changed daily); use filtered gases (0.2 μm membrane filters on all process gas lines); reduce oxidation temperature for better oxide quality (900-950°C dry O₂ yields lower defect density than 1000-1100°C) |
| Diode high reverse leakage | Incomplete junction formation (diffusion too shallow), surface contamination (ionic residues), or crystal defects (dislocation density >10³ cm⁻²) | Extend diffusion time (increase junction depth to 2-5 μm minimum for reliable junction); improve surface passivation (grow 0.5-1.0 μm thermal oxide over junction after metallization patterning); check wafer quality (reject wafers with slip lines visible under collimated light) |
| Thyristor false triggering | dv/dt exceeds rating (typically 50-200 V/μs), or noise on gate terminal (>5 mV RMS) | Add snubber circuit (RC across device: R = 10-100 Ω, C = 0.1-1.0 μF, sized so RC time constant > dv/dt × Vdrm limit); shield gate trace with grounded guard ring; add gate-cathode resistor (100-1000 Ω) to shunt noise current away from gate-cathode junction |
| Thermal runaway (power device) | Positive temperature coefficient of leakage current (doubles every 8-10°C for silicon), combined with inadequate heatsink (Rth >5 °C/W) | Ensure adequate heatsinking (calculate Tj = Ta + Pd × Rth_total, maintain Tj < 150°C with 25°C margin); add emitter/source ballast resistance (0.1-0.5 Ω) in series with each parallel device for current sharing; derate power dissipation by 1.5% per °C above 25°C ambient |
| MOSFET failure during switching | Voltage spike from inductive load (V_spike = L × dI/dt) exceeds Vds(max), or parasitic BJT latchup from dv/dt >50 V/ns | Add freewheeling diode across inductive load (fast recovery type: trr <100 ns); use avalanche-rated MOSFET (rated for repetitive avalanche energy, typically 100-500 mJ); increase gate resistor (5-50 Ω series) to slow switching (reduces spike but increases switching loss by 10-30%) |
| BJT secondary breakdown | Localized hot spot from non-uniform current distribution at high Vce (>50 V) and high Ic simultaneously, creating thermal runaway in a small region | Operate within safe operating area (SOA) curve from datasheet (derate linearly above 25°C case temperature); use MOSFETs instead for linear applications (no secondary breakdown mode); add emitter ballast resistors (0.1-0.5 Ω per device) for parallel operation to force current sharing |
| Wire bond failure in package | Thermal cycling fatigue (CTE mismatch between Au wire at 14.2 ppm/°C and Al pad at 23 ppm/°C), or moisture-induced corrosion of Al pad | Use gold wire for high-reliability (Au-Au interface eliminates intermetallic growth); control molding compound moisture (bake compound pellets at 125°C for 24 hours before use if stored >30 days at >50% RH); follow MSL handling procedures (bake packaged devices at 125°C for 24 hours before board-level reflow if floor life exceeded) |
| Contact resistance drift on Test | Aluminum metallization oxidation (Al₂O₃ grows 2-5 nm/year at 25°C, increases probe contact resistance) | Use probe cards with tungsten or beryllium-copper tips (penetrate oxide with 50-150 grams overdrive force); clean probe tips every 10,000 touchdowns with isopropyl alcohol; verify contact resistance <1 Ω per probe before each wafer lot |
