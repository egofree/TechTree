...
> **Node ID**: machine-tools.abrasives
...
- [Bearings](./bearings.md) — bearing design and manufacture
...
- [Forming](./forming.md) — forging and rolling operations
...
|---------|---------------|----------|
| Grinding wheel fails ring test (dull thud instead of clear 2-3 second sustain) | Crack in vitrified bond from thermal shock (cooling rate exceeded 30-50°C/hour during manufacture) or impact damage | Discard the wheel immediately — cracked wheels can explode at rated speed (1500-2100 m/min peripheral); never attempt to use a wheel that fails ring test; replace with new wheel and verify with ring test before mounting |
| Lapped surface non-flat by >0.005 mm over 50 mm (target 0.0005 mm / 0.5 μm) | Lapping plate worn beyond 0.005 mm deviation, or uneven hand pressure during figure-8 motion | Recondition plate by lapping three plates together (A laps B, B laps C, C laps A); apply firm, even downward pressure of 5-15 N for 50 mm workpiece; rotate workpiece 90° every 30-60 seconds for uniform removal |
| Carbon steel tool bit loses edge after <15 minutes cutting mild steel | Temper drawn during grinding (tool overheated — no water quench every 10-15 seconds), or cutting speed exceeds 5-10 m/min limit for carbon steel | Re-harden: heat to 780-820°C (cherry red), quench in water or 10% brine with agitation; temper at 200-250°C (straw color) for 30 minutes; keep tool cool during grinding — dip in water every 10-15 seconds; limit cutting speed to 5-10 m/min for steel |
| Thread pitch diameter out of tolerance (±0.05 mm) on M8×1.25 | Change gear ratio incorrect, or compound rest angle not set to 29-30° (half thread angle + 0.5°) | Verify change gear ratio: (pitch to cut / leadscrew pitch) × (stud/lead teeth); for M8×1.25 with 4 mm leadscrew, use 25-tooth stud / 80-tooth lead (25/80 = 0.3125 = 1.25/4); set compound rest to 29-30°; check with thread micrometer (0.01 mm resolution) |
| Synthetic SiC abrasive does not scratch glass (Mohs 5.5) — batch under-reacted | Electric arc furnace temperature below 2200-2500°C, or insufficient hold time (<36 hours at temperature) | Increase current to 2000-4000 A at 100-200 V; extend furnace cycle to 36-48 hours; verify core temperature with optical pyrometer; only use inner zone crystalline mass (within ~200 mm of core) — outer zone is recycled |
| Sieve mesh stretches, passing >5% of sample to next finer grade | Wire mesh worn from abrasion or frame loose (copper tacks at 15 mm spacing pulled out) | Replace mesh — use 0.15-0.50 mm diameter brass wire (harder wire lasts longer); re-secure frame with new copper tacks at 15 mm spacing; recalibrate monthly with known-weight graded sample |
| Tap breaks off inside M8 hole (brittle HSS at 62-65 HRC) | Tap tilted during starting (not perpendicular to surface), or cutting torque exceeded 2-5 Nm limit for M8 in mild steel | Start tap with a guide block to maintain perpendicularity; use taper tap first (7-10 chamfered threads, lead angle ~4°); apply sulfurized cutting oil; turn 1/4 turn forward then 1/8 turn back to break chip; if broken, dissolve with nitric acid or EDM extract |
| Grinding wheel vibration marks on workpiece (periodic waviness matching wheel RPM) | Wheel unbalanced, or wheel out-of-round (runout >±0.02 mm from dressing) | Mount wheel on balancing arbor; dress periphery with diamond dresser to ±0.02 mm runout; balance using adjustable weights in the flange; check arbor bore fit (H7 tolerance, ±0.015 mm for 20 mm bore); verify wheel speed matches rated RPM |
