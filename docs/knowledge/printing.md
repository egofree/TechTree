...
**Rag paper production** (pre-industrial detail):
...
**Woodblock printing** (China, ~700 CE; Europe, ~1400 CE):
...
**Early movable type** — Bi Sheng's ceramic type (China, ~1040 CE):
...
**Metal movable type** — Gutenberg's system (Mainz, ~1440 CE):
...
**Woodcut illustration** (concurrent with woodblock text printing):
...
- **Printing cost distribution** (typical job): Paper ~50% of total cost, labor (typesetting, press operation, binding) ~25%, ink ~5%, overhead (press depreciation, shop rent, power) ~20%. Ink is a surprisingly small fraction of total cost, but its properties (viscosity, tack, drying time) critically affect print quality and press productivity.
