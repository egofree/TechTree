# Siege Engineering & Military Logistics

> **Node ID**: defense.siege-warfare
> **Domain**: Defense & Military Engineering
> **Dependencies**: None (root capability)
> **Enables**: None (leaf capability)
> **Timeline**: Years 5-50+
> **Outputs**: siege_engines, cannons, military_logistics, artillery, field_fortifications

### Battering Ram (Years 5-15)

**Basic ram**: Heavy timber beam (6-10 m × 30-50 cm diameter, 200-500 kg), typically oak or ash. Suspended horizontally from timber frame by ropes or chains, swung by 10-30 men. Impact energy per swing: 2000-6000 J (mass × velocity² / 2). Beam tip reinforced with iron cap (5-10 kg) to prevent splitting and concentrate force on gate or wall.

**Covered ram (testudo/turtle)**: Wooden shed on wheels protecting ram crew from missiles and boiling liquids dropped from walls. Shed dimensions: 4-6 m long × 2.5-3.0 m wide × 2.0-2.5 m high. Shed roof: layered construction — planks + rawhide + earth (50-100 mm) for fire resistance and missile protection. Covered by fresh animal hides during approach to resist incendiary attack. Weight: 1000-2000 kg total (structure + ram). Moved on 4-6 wheels by 20-40 men pushing from inside.

**Effectiveness**: Battering ram breaks wooden palisade gate in 15-30 minutes of sustained ramming. Iron-shod ram breaches stone wall only if wall is thin (<1.5 m) or poorly mortared — typically used against gates rather than walls. Counter: iron-bound gates, earthen fill behind gates, portcullis, murder holes for dropping projectiles onto ram crew.

### Siege Towers (Years 10-25)

**Construction**: Multi-story wooden tower on wheels. Dimensions: 10-20 m height × 4-6 m wide × 4-6 m deep (must be taller than target wall by 2-3 m). Stories: 3-5 levels. Ground floor: protected approach gangway. Upper floors: archers/javelinmen firing platforms. Top floor: drawbridge to bridge onto wall walk. Weight: 5000-15,000 kg.

**Materials**: Timber frame (oak or pine), plank siding, rawhide or metal sheet facing for fire resistance. Wheels: 4-6 heavy spoked wheels, 50-80 cm diameter, iron-tired axles. Assembly time: 2-6 weeks with 50-100 workers (prefabricated in sections near siege site, assembled under cover of siege screens).

**Approach**: Tower pushed toward wall on prepared ramp of compacted earth (to level ground and reduce approach distance). Ramp construction: 10-20 m wide, 2-5% grade, packed earth — 5000-15,000 m³ of earth for 100 m approach at 3 m height. During approach, defenders target tower with incendiary projectiles (Greek fire, burning pitch). Counter-counter: fire-resistant cladding, wet hides, assault archers suppressing wall defenders.

**Siege tower vulnerability**: Requires flat or gently sloping approach terrain. Useless against concentric castles (outer wall blocks approach to inner wall). Obsolete once cannon available — large stationary wooden target destroyed by artillery fire in minutes.

### Torsion Artillery (Years 10-25)

**Catapult (mangonel)**: Single-arm torsion engine. Arm: 3-5 m wooden beam with rope bundle torsion spring at pivot. Sling or bucket at arm end holds stone projectile (10-50 kg). Torsion spring: sinew and hair rope bundle (150-300 mm diameter) wound tight around arm pivot, pretensioned by winch. Range: 100-300 m. Rate of fire: 1-2 shots per 10 minutes (rewinding torsion spring is slow). Crew: 8-20 men.

**Ballista**: Two-arm torsion engine (giant crossbow). Two torsion springs (one per arm) drive a pair of arms forward, propelling a bolt or stone along a guided track. Bolt: 60-120 cm, iron or steel head, wooden shaft, 2-5 kg. Stone: 5-30 kg. Range: 200-400 m (bolt), 150-300 m (stone). Accuracy: can hit a man-sized target at 100-150 m with experienced crew and calibrated torsion spring tension.

**Torsion spring construction**: Rope bundles of animal sinew (preferred — high elastic energy storage) or human hair. Bundle dimensions: 150-300 mm diameter × 400-800 mm length. Pretension: rope wound to specific torque (measured by lever arm weight — empirical calibration). Critical maintenance: sinew ropes lose tension when wet (humidity, rain) — must be kept dry under canvas covers. Damaged ropes replaced by skilled rope-makers (see [Textiles: Rope](../textiles/index.md)).

### Trebuchet (Years 10-30)

**Counterweight trebuchet**: Gravity-powered siege engine with massive superiority over torsion engines — simpler construction, more reliable, greater range and payload. Beam: 5-10 m long, mounted on pivot at 1:4 to 1:6 ratio (short counterweight arm : long throwing arm). Counterweight: 5-15 tonnes (stone, lead, or iron-filled wooden box) attached to short arm. Sling at long arm end holds projectile. Trigger mechanism releases sling at optimal angle for maximum range.

**Performance parameters**:
- **Small trebuchet**: 3-tonne counterweight, 6 m beam, 30-50 kg projectile, 100-150 m range.
- **Medium trebuchet**: 8-tonne counterweight, 8 m beam, 80-150 kg projectile, 150-250 m range.
- **Large trebuchet**: 15-tonne counterweight, 10-12 m beam, 100-300 kg projectile, 200-300 m range.

**Projectile types**: Stone (limestone or sandstone, roughly spherical, 30-300 kg — cheapest, most common). Incendiary: clay pot filled with Greek fire (naphtha + sulfur + quicklime, 5-10 kg — ignites on impact). Dead animals: biological warfare (fling plague-infected carcasses over walls — historically documented at sieges).

**Rate of fire**: 1 shot per 5-15 minutes (resetting counterweight, loading projectile, aiming). Crew: 12-50 men for operation (hauling counterweight back to firing position is the labor bottleneck). Aiming: adjust sling release point (release earlier = higher trajectory + shorter range, release later = lower trajectory + longer range). Trained crews achieve 10-20 m accuracy at 150 m range.

**Construction time**: 2-4 weeks with 20-50 workers for medium trebuchet. Timber requirements: 5-10 m³ of oak or elm for beam and frame, plus rope and iron fittings. Must be assembled on-site (too large to transport assembled).

### Mining & Counter-Mining (Years 10-30)

**Underground mining (sapping)**: Tunnel dug under wall foundation. Tunnel dimensions: 1.0-1.5 m wide × 1.5-2.0 m high (minimal cross-section to reduce cave-in risk before deliberate collapse). Timber shoring: pit props at 0.5-1.0 m intervals. When tunnel reaches under wall foundation, burn props to collapse tunnel → wall settles and cracks, or breaches entirely.

**Fire-setting for tunnel excavation**: Build fire at tunnel face to heat rock, then dash with cold water (or vinegar) → thermal shock fractures stone. Fumes require ventilation: bellows-driven air through timber-lined ventilation shaft. Tunnel advance rate: 0.5-2.0 m per day (depending on rock hardness).

**Counter-mining**: Defenders dig counter-tunnels to intercept besieger's mine. Detection: listening for mining sounds (picking, scraping, timber creaking) through gallery walls — using shielded ear against wall at night when surface noise minimal. Counter-mine breakthrough: flood besieger's tunnel with water, collapse it by undermining its props, or attack miners directly through the breach.

### Cannon Artillery (Years 20-50)

**Early cannon (13th-14th century)**: Wrought iron or cast bronze barrel. Bore: 50-200 mm diameter. Barrel length: 6-12× bore diameter. Propellant: black powder (see [Explosives](../chemistry/explosives.md) for composition: 75% KNO₃, 15% charcoal, 10% sulfur). Projectile: stone sphere (cheaper) or iron sphere (denser, more kinetic energy). Muzzle velocity: 300-500 m/s.

**Cannon barrel manufacture**:
- **Wrought iron (hoop and stave)**: Longitudinal iron bars (staves) wrapped with iron hoops, forge-welded. Barrel wall: 15-30 mm iron. Risk: welded seams can burst under pressure. Maximum safe powder charge: limited by weld quality.
- **Cast bronze**: Bronze alloy (88-92% Cu, 8-12% Sn) cast in clay mold around core → smooth, seamless bore. Bronze barrel superior to wrought iron: no weak seams, better pressure containment, smoother bore = more accurate shot. Weight: 500-5000 kg for siege cannon.

**Cannon bore progression by era**:
| Period | Bore (mm) | Barrel Weight (kg) | Projectile Weight (kg) | Powder Charge (kg) | Effective Range (m) |
|--------|-----------|-------------------|----------------------|-------------------|-------------------|
| 1300s | 50-100 | 200-1000 | 2-10 | 0.5-3 | 100-200 |
| 1400s | 80-200 | 500-5000 | 5-50 | 2-15 | 200-400 |
| 1500s | 100-300 | 1000-15000 | 10-100 | 5-30 | 300-600 |
| 1600s+ | 80-200 (standardized) | 500-5000 | 4-30 | 2-10 | 400-1000 |

**Siege bombard**: Oversized cannon for breaching walls. Famous example: Mehmed II's Great Bombard (1453, siege of Constantinople): ~750 mm bore, 8 m barrel, cast bronze, 600 kg stone shot, 50 kg powder charge, range ~1500 m, rate of fire 3 shots per day (barrel required cooling). Breached Theodosian Walls (5 m thick) after 6 weeks of bombardment.

**Cannonball vs wall**: Impact energy = ½mv². A 25 kg iron ball at 400 m/s delivers 2,000,000 J (2 MJ). This energy fractures stone and compresses mortar at impact point, creating a crater 30-60 cm deep × 50-100 cm diameter per shot. Repeated strikes at same point create breach: 50-200 shots to open 2-3 m wide breach in 2 m stone wall.

### Military Logistics (Years 0-50+)

**Supply requirements per soldier per day**:
- Grain/flour: 1.0-1.5 kg (3,500-5,000 kcal)
- Water: 4-5 L (drinking and cooking; 10-15 L in hot climate)
- Firewood: 1-2 kg (cooking, warmth)
- Ammunition (firearms era): 60 rounds (180 g powder + 2 kg lead)
- Fodder (per horse): 5-10 kg hay + 2-5 kg grain + 30-40 L water

**Baggage train**: 1000-soldier force with 200 horses requires daily:
- 1000-1500 kg grain, 4000-5000 L water, 1000-2000 kg firewood for soldiers
- 1000-2000 kg hay + 400-1000 kg grain + 6000-8000 L water for horses
- Total daily requirement: 3-6 tonnes food + 10,000-13,000 L water
- Transport: 20-40 wagons (each 500-1000 kg payload) or 100-200 pack animals per day's supply

**March rate**: Infantry: 15-25 km/day on roads, 8-15 km/day cross-country. Cavalry: 25-40 km/day. Baggage train: 15-20 km/day (limits entire army pace). Maximum effective campaign radius from supply base: 200-300 km (10-15 days march out, retain sufficient supplies for return).

**Supply chain vulnerability**: An army that cannot feed itself disintegrates in 3-5 days. Historical campaigns lost more soldiers to starvation and disease than combat (ratio typically 3:1 to 10:1). Logistics infrastructure (roads, wagons, depots) determines military capability more than weapons — see [Transport](../transport/index.md) for road and railway construction.

### Field Fortifications (Years 5-50+)

**Marching camp**: Entrenched camp constructed at end of each day's march (Roman model). Standard layout: rectangular, 4 gates, ditch (1.5 m wide × 1.0 m deep), rampart (0.5-1.0 m high from ditch spoil), palisade (wooden stakes carried by each soldier). Construction time: 2-4 hours for 1000-man camp. Provides: defended perimeter against night attack, organized interior (tents in regular grid, commander's tent on central axis), secure base for next day's operations.

**Redoubt**: Small enclosed field fortification (20-50 m square) with earthen parapet and ditch. Cannon or musket positions on each face. Construction: 500-2000 worker-hours. Provides strongpoint in defensive line or fortified position for artillery battery.

**Abatis and entanglement**: Felled trees, sharpened stakes, and barbed wire creating obstacle zone 20-50 m deep in front of defensive position. Purpose: delay attacking infantry 3-10 minutes under defensive fire — converts open ground into kill zone. See [Fortifications](fortifications.md) for detailed obstacle specifications.

### Siege Timeline & Resource Summary

| Phase | Duration | Key Activities | Resources Required |
|-------|----------|---------------|-------------------|
| Investment | 1-7 days | Surround fortress, cut supply, build siege camp | 5000-20000 soldiers |
| Approach works | 2-8 weeks | Dig approach trenches, build batteries, construct siege engines | 1000-5000 laborers, timber, tools |
| Bombardment | 1-6 weeks | Cannon fire to breach walls, mining under foundations | 10-50 cannon, 5000-20000 shot, 10000-50000 kg powder |
| Assault | 1-3 days | Storm breach, scale walls, street fighting | Assault force 500-5000 |
| Total typical siege | 4-16 weeks | All of the above | 10000+ soldiers, massive material stockpile |

### Siege Mining Engineering

**Tunnel construction**: Mine gallery dimensions optimized for speed and stability. Width: 1.0-1.5 m (one man working width). Height: 1.5-2.0 m (crouching/working height). Timber shoring: vertical posts (100-150 mm diameter) at 0.5-1.0 m intervals, horizontal cap piece connecting posts. Side boards (planks) prevent wall collapse. Critical: miners work in silence near enemy walls — any noise detected by counter-miners.

**Ventilation**: Single-entry tunnel fills with CO₂ (heavier than air, pools at floor) and exhaled breath. Bellows-driven ventilation through wooden or clay pipe (10-15 cm diameter). Fresh air pumped in from surface, stale air forced out through pipe to tunnel face. Fire-setting accelerates ventilation need — combustion gases (CO, CO₂, SO₂) are immediately lethal in confined space.

**Explosive mining (post-gunpowder)**: Gunpowder charge placed in chamber excavated under wall foundation. Charge size: 500-5000 kg depending on wall thickness and foundation depth. Tamping: chamber backfilled with compacted earth and sand to contain blast energy (directed upward into wall foundation). Ignition: slow match fuse or (later) electrical detonation. Effect: massive underground explosion creates crater, wall foundation loses support, section collapses into crater. Safer and faster than fire-setting method.

### Artillery Battery Construction

**Gun platform**: Level platform (3-5 m × 3-5 m per gun) constructed of compacted earth and timber, 1-2 m above surrounding terrain for clear line of fire over approach works. Platform must support cannon weight (500-5000 kg) plus recoil forces — timber cribbing with earth fill, anchored by stakes. Recoil absorption: earth and timber platform allows gun to recoil 1-3 m, then manhandled back into position. Rate of fire for siege cannon: 6-12 shots per hour (reload + aiming + barrel cooling).

**Powder magazine**: Bomb-proof storage for gunpowder. Earth-bermed timber or stone structure, minimum 50 m from gun positions. Powder stored in barrels (50-100 kg each) on raised timber platforms (keep dry). Strict safety: no naked flames, no metal tools that spark (use wooden mauls and copper-tipped implements), single entrance with deflection wall (blast directed sideways if magazine detonates). Magazine capacity: 5000-50000 kg powder for sustained siege.

**Shot furnace**: Furnace for heating solid iron shot to red heat before firing — "hot shot" ignites wooden structures and ships. Furnace: brick-lined, charcoal-fired, heats 12-25 kg iron balls to 600-800°C in 20-30 minutes. Handling: iron tongs to load hot shot into cannon (wet wad between powder charge and hot shot prevents premature ignition). Devastating against wooden ships and siege engines — penetrates wood, ignites from inside. Cannot be used with explosive shells (premature detonation risk).

### Naval Siege Warfare

**Blockade**: Naval force prevents supply by sea — starving fortress into surrender. Requires naval supremacy in local waters. Blockade squadron: 3-10 ships depending on harbor size. Blockade runners intercepted by patrol vessels. Effectiveness: complete naval blockade reduces defender's supply to internal stores only — surrender typically follows in 2-6 months if no relief force arrives.

**Bombardment from sea**: Ships-of-the-line carry 30-120 guns each (24-36 pounder cannon). Broadside weight: 200-500 kg of iron shot per volley. Range: 500-1000 m (ships carry heavier guns than field artillery). Ship bombardment of coastal fortifications: limited by ship motion (gun platform unstable) and fortification return fire (fortress guns on stable platform, more accurate). Historically, shore fortifications usually win gunnery duels against ships.

### Limitations

- **Logistics as primary constraint**: Every siege is a race between starving out the defenders and running out of supplies yourself. Besieger must maintain supply lines while cutting defender's. Disease (dysentery, typhus) kills 2-10× more soldiers than combat in sieges.
- **Weather dependency**: Winter sieges extremely difficult (frozen ground prevents mining, supply lines disrupted, troop morale collapses). Campaign season: April-October in temperate climates.
- **Engineering bottleneck**: Siege engines and cannon require skilled craftsmen (carpenters, blacksmiths, foundrymen). Loss of engineering crew to disease or sortie can halt siege entirely.
- **Defender advantages**: Interior lines, fortified positions, prepared supplies, known terrain. Historically, 3:1 attacker advantage required for reasonable chance of success against well-fortified position with competent garrison.

### See Also

- **[Fortification Engineering](fortifications.md)** — What siege engines attack
- **[Explosives & Propellants](../chemistry/explosives.md)** — Black powder for cannon propellant
- **[Transport & Logistics](../transport/index.md)** — Road construction, wagons, railways for military supply
- **[Mining Engineering](../mining/index.md)** — Tunneling techniques for siege mining
- **[Textiles: Rope](../textiles/index.md)** — Rope for trebuchet slings, torsion springs, and hoisting

 *Part of the [Bootciv Tech Tree](../index.md) • [Defense & Military](./index.md) • [All Domains](../index.md)*
