...
- **Cloth beam** (front): 8 cm diameter × 120 cm hardwood roller. Iron ratchet and pawl mechanism on one end to maintain cloth tension. Canvas or webbing wrapped around beam to grip the finished cloth.
- **Warp beam** (back): 8 cm diameter × 120 cm hardwood roller. Similar ratchet mechanism. Wooden pegs or cords (leasing cords) hold warp ends evenly spaced.
- **Breast beam** (front, above cloth beam): 5 × 5 cm × 120 cm. The weaver's bench sits behind this beam. Cloth passes over it on its way to the cloth beam.
...
- **Treadles** (2-8): 5 × 10 cm × 60 cm hardwood planks, pivoting at the rear on iron pins. Connected to shafts via tie-up cords. Weaver depresses treadle with foot → shaft rises → shed opens.
...
- **Dent spacing**: Measured in dents per inch (dpi) or dents per 10 cm. Common sizes: 8, 10, 12, 15, 20 dpi. Finer cloth = higher dpi. A 12 dpi reed with 1 warp thread per dent produces 12 ends per inch (epi). Two threads per dent → 24 epi (used for fine cloth).
...
- **Twill**: Over 2, under 1 (or 2/2 twill). Diagonal rib pattern visible on fabric surface. Drapes better, more flexible. For clothing, work garments. 4 shafts for basic twills; point twills and herringbone use 4-8 shafts. Heddle lifting sequence advances one shaft each pick, creating the diagonal.
- **Satin**: Over 4+, under 1. Smooth face, less durable. Long floats catch and snag. For fine fabrics. Requires 5+ shafts. Floats distributed so no two adjacent in either direction.

**Complex weave structures**:
- **Basket weave (2/2 tabby)**: Two or more warp threads work as one, crossing over and under two or more weft threads. Looser, more flexible than plain weave. Useful for mats, heavy canvas. 2 shafts.
- **Herringbone twill**: Twill direction reverses at regular intervals (every 8-16 threads), creating a V-pattern. Requires 4 shafts. Classic for suiting fabrics. The reversal point creates a characteristic "break" line in the cloth.
...
1. **Calculate warp length**: Finished cloth length + 10-15% for take-up (yarn shortens as it interlaces with weft) + 30-60 cm loom waste (tied to beams, cannot be woven). For a 2-meter cloth length, warp ~2.5-2.8 meters.
2. **Calculate warp width**: Finished cloth width + 5-10% for draw-in (cloth narrows as weft packs). For 60 cm finished width at 20 epi, warp ~1300 threads over ~66 cm.
3. **Measure warp on warping board or pegs**: Wooden board with pegs set at fixed distance (e.g., 1 meter apart). Walk yarn around pegs in figure-8 pattern, counting each circuit. Cross (lease) at one end keeps threads in order — critical for threading heddles correctly.
...
- **Stick shuttle**: Flat piece of hardwood (30-50 cm), notched at both ends to hold weft yarn. Simplest. Wind yarn in figure-8 pattern. Suitable for frame looms and narrow work. Slow to pass — must be threaded through shed by hand.
- **Boat shuttle**: Boat-shaped wooden body (25-35 cm) with hollow center holding a removable bobbin (small cylinder wound with weft yarn). Yarn feeds through a hole in shuttle side. Flies through shed on a treadle loom — much faster than stick shuttle. Requires bobbins wound evenly (use bobbin winder — small hand-cranked spindle). Shuttle weight: 50-150 g depending on size.
...
| Weight (oz/yd²) | Thread Count | Application |
|-----------------|-------------|-------------|
| 6-8 oz | 20-24 ends/inch | Light sails, clothing, bags |
| 10-12 oz | 16-20 ends/inch | Medium sails, tents, awnings |
| 14-18 oz | 12-16 ends/inch | Heavy sails, tarpaulins, floor cloths |
| 20+ oz | 10-14 ends/inch | Sail reinforcements, pallet covers, industrial |
...
- **Fulling**: Beat cloth in warm water (40-50°C) with fuller's earth (clay) or stale urine (ammonia source). Wool fibers felt and interlock → cloth shrinks 15-25%, becomes denser and warmer. Fuller's earth (aluminum silicate clay) absorbs oils and acts as a mild detergent. For detailed finishing processes, see [Finishing](finishing.md).
- **Washing/scouring**: Remove sizing, spinning oils, and dirt from loomstate cloth. Wash in hot water (60-80°C) with soap or soda ash. Rinse thoroughly. Cotton and linen shrink 3-8% in the first wash.
...
- **Cartwright power loom (1785)**: First practical power loom, powered by steam engine via belt drive. Automated the three basic loom motions: shedding (raising warp), picking (throwing shuttle), and beating (packing weft). Early versions produced 2-3× a hand weaver's output. Required robust iron frame to withstand the forces of mechanized beating.
- **Improvements through 1820s-1840s**: Roberts' self-acting loom added automatic shuttle changing and warp let-off. Houldsworth's loom improved the shedding mechanism. By 1850, a single power-loom weaver tended 2-4 looms simultaneously. Production: 20-40 m²/hour per loom — 10-20× handloom rate.
...
- **Weave defects**: Skipped picks (missing weft thread — appears as a thin horizontal line), broken ends (broken warp thread — thin vertical line), floats (yarn floating over surface where it should be interwoven — indicates broken heddle or mis-threaded shaft), slubs (thick lumps in yarn — cosmetic, not structural).
- **Tension defects**: Draw-in (cloth narrower at one end — uneven warp tension), bagginess (loose area — slack warp or weft), ridges (visible warp or weft bands — inconsistent beating pressure).
- **Measurement**: Check cloth width (compare to specification — ±2% tolerance), length (measure from warp and cloth beam counters), weight (grams per square meter — ±5% tolerance), and ends/picks per inch (count with a pick glass — 10× magnification lens mounted in a frame with a 1-inch opening).
- **Grading system**: Assign cloth to quality grades based on defect count per 100 linear meters. Grade A: 0-3 major defects (functional impairment). Grade B: 4-8 major defects. Grade C: 8+ major defects — usable only for low-grade applications. Penalties assessed on a point system — 1 point per defect, 4 points maximum per linear yard regardless of defect count.
...
- **Dust and fiber inhalation**: Cotton and fiber dust generated during weaving causes respiratory irritation (byssinosis — "brown lung" — with chronic cotton dust exposure). Ventilation in enclosed weaving areas. Respirators for cotton-heavy environments.
...
**Frame assembly** (for a 1.2-1.5 m weaving width):
- Side posts: 10 × 10 cm × 140 cm, mortised for cross-members. Stand on flat feet to prevent tipping.
- Cross-members: 5 × 10 cm, bolted through posts with 12 mm iron bolts or secured with wedged through-tenons for a no-metal-required version.
- Cloth beam and warp beam: 8-9 cm diameter × 130-150 cm rollers with iron ratchet and pawl mechanisms. The ratchet must hold firmly — any slippage releases warp tension and ruins the weave. Minimum 12 teeth on the ratchet for adequate tension adjustment.
- Total frame width: 30-40 cm wider than the weaving width (15-20 cm on each side for the frame and mechanism).

**Treadle mechanism with 2-8 shafts**:
- Each shaft is connected to one or two treadles via tie-up cords. The tie-up pattern determines the weave structure. For plain weave: 2 shafts, 2 treadles. For twill: 4 shafts, 4-6 treadles. For complex patterns: 8+ shafts.
- Treadle springs or counterweights return the treadles to the neutral position after depression. Without return springs, the weaver must manually lift each treadle, slowing the rhythm. Elastic cord (later development) or rubber band springs work; traditional looms use gravity counterweights on the shafts.

**Heddle making detail**:
- **Wire heddles**: 5-12 dents per cm (12-30 per inch) depending on yarn fineness. Fine cotton (Ne 40-60) requires 10-12 dents/cm with 0.5 mm wire. Coarse wool requires 4-6 dents/cm with 0.8 mm wire. A heddle bending jig (wooden block with steel pins) produces uniform heddles in seconds. Each heddle has a centered eye (3-5 mm opening) through which the warp thread passes. A 100 cm wide warp at 20 epi requires 800 heddles per shaft — 1,600 for a 2-shaft plain weave loom.
- **String heddles**: For field-expedient construction or low-production use. Strong linen or cotton cord, knotted with a Midway knot or heddle knot to create the eye. Each heddle takes 2-3 minutes to tie. A complete set for a 100 cm loom at 20 epi requires 25-40 hours of knotting. String heddles stretch with use and must be replaced periodically.
...
**Twill** (2/2 interlacing): Each weft thread passes over two warp threads and under two, with the interlacing point shifting one thread on each successive pick. This creates the characteristic diagonal rib visible on the fabric surface. The diagonal angle depends on the ratio of warp density to weft density: equal densities produce a 45° diagonal. Twill drapes better than plain weave because the longer floats allow more yarn mobility. 4 shafts minimum for basic 2/2 twill. Variants: 1/3 twill (steeper diagonal, one side smoother), 2/1 twill (unbalanced, one side dominant).
...
**Herringbone**: A twill variant where the diagonal direction reverses at regular intervals (every 8-16 threads), creating a V-pattern resembling fish bones. The reversal point creates a characteristic "break" line where the twill changes direction. Herringbone is one of the most common suiting fabric patterns. Requires 4 shafts for basic versions.
...
**Wet finishing** (loom-state cloth): Cloth fresh off the loom contains spinning oils, sizing residue, and loose fiber ends. The weave structure has not yet "settled" — the yarns are straight and parallel, not fully interlocked. Wet finishing remedies this:
...
|-----------|------|-----------|-------------------|-------|
| Hand loom | - | 30-40 | 1 | Skilled weaver, all motions manual |
| Cartwright power loom | 1785 | 60-80 | 1 | Steam-powered, iron frame |
| Improved power loom | 1850 | 120-150 | 1 tending 2-4 | Roberts self-acting, automatic shuttle |
| Northrop automatic | 1894 | 150-200 | 1 tending 12-16 | Automatic bobbin change |
...
- **Broken ends**: A broken warp thread creates a thin vertical line where the weft is not held in place. Minor breaks (1-2 per meter) are acceptable in Grade B fabric. The broken end can be mended by hand (reweaving a replacement thread through the existing structure) but this is labor-intensive.
- **Pulls and snags**: A loop of yarn pulled to the surface, usually from catching on rough machinery. Minor cosmetic defect. Trim flush with scissors.
