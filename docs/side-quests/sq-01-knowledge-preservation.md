# SQ 1: Knowledge Preservation &amp; Education System

**Starts**: Phase 1
**Priority**: Critical — without this, the effort dies with the first generation.

## Problem

All the technical knowledge in the world is useless if it cannot be transmitted across generations. A bootstrapping effort spanning 50-200+ years will outlive every original participant. Systematic knowledge preservation is non-negotiable.
...
**Writing media progression**:
- **Clay tablets** (Phase 1): Write in wet clay with wooden stylus (wedge-shaped tip → cuneiform-like). Air dry or fire for permanence. Durable (thousands of years). Heavy, bulky. Good for permanent records.
- **Waxed wooden tablets** (Phase 1): Recessed wooden board filled with beeswax. Write with metal or bone stylus. Erasable by smoothing wax. Reusable. Good for drafts, calculations, temporary records.
- **Parchment** (Phase 1-2): Animal skin (sheep, goat, calf) soaked in lime (Ca(OH)₂) for 3-10 days to remove hair and fat. Stretch on frame. Scrape with lunellar knife (crescent-shaped blade) to uniform thickness. Rub with pumice to smooth surface. Dust with chalk to absorb grease. Cut to sheets. Durable (centuries), expensive (one sheep = ~2-4 pages). Ink: iron gall (iron sulfate + tannin from oak galls + gum arabic binder — permanent black/brown ink).
- **Paper** (Phase 2-3): See paper production section below.
...
**Chemical pulping** (kraft process, Phase 5+):
...
**Papermaking** (hand process, Phase 2-3):
...
5. **Drying**: Hang sheets in drying loft (warm, ventilated room) for 12-24 hours. Or pass through heated dryer cans (Phase 4+ — steam-heated iron cylinders, paper passes over surface, moisture evaporates).
...
**Production rate**: Hand process: 100-300 sheets/day per worker. Machine process (Phase 4+): 100-1000 m/minute — essentially unlimited.

### Printing Press

**Movable type**:
- **Type metal**: Alloy of lead (80-85%), antimony (10-15%), and tin (3-5%). Antimony makes type hard and reduces shrinkage on cooling. Tin improves fluidity for casting. Melt at ~300°C.
- **Type casting**: Brass matrix (mold) for each character. Inject molten type metal into matrix with hand casting ladle or simple plunger mold. Each type piece is a rectangular block ~2 cm tall with raised letter on one face. Cast hundreds of each letter. Organize in type cases (wooden trays with compartments for each letter — larger compartments for common letters: e, t, a, o, i, n, s).
- **Typesetting**: Compose text by selecting individual type pieces and arranging them in a composing stick (metal tray, adjustable width). Read text backwards (type prints mirror image). Insert spacing material (leads — thin metal strips) between lines. Transfer composed type to galley (flat tray) for proofing. Proof → correct → lock up in forme (wooden or metal frame with wedges (quoins) to hold type tight).
- **Press construction**: Wooden or iron frame. Ink slab (flat stone or metal surface). Ink balls (leather pads stuffed with wool, mounted on wooden handles — apply ink to type). Platen (flat board that presses paper against inked type). Screw mechanism or lever to drive platen. Bed (flat surface where forme sits). During Phase 3, iron construction replaces wood for rigidity → more consistent impression → higher print quality.
...
4. **Production**: 200-500 impressions/hour for hand press. 1000-4000 impressions/hour for rotary press (Phase 4+). Print one side (folio), let dry, print other side. Multiple colors require separate press runs with different type formes, registered to align.
...
| Microfilm (Phase 8+) | 100-500 years | Requires reader |

### Digital Knowledge Storage (Phase 8+)
...
| Phase | Contribution |
|-------|-------------|
| Phase 1 | Writing system, clay tablets, wax tablets, basic records, apprenticeships |
| Phase 2 | Metallurgical recipes, tool designs, parchment production |
| Phase 3 | Machine shop manuals, metrology standards, paper production, printing press |
| Phase 4 | Electrical engineering knowledge base, rotary printing press |
| Phase 5 | Chemical processes, safety protocols, chemical pulping for paper |
| Phase 7-8 | Semiconductor fabrication manuals, cleanroom protocols, digital storage |

## Key Deliverables

- Phonetic writing system with technical notation (units, elements, mathematics)
- Paper production capability (mechanical or chemical pulping → hand or machine papermaking)
- Movable type printing press operational
- Technical library with redundant copies in fireproof facilities
- Apprenticeship framework with defined skill tracks
- Standardized technical drawing format
- Complete failure log and material property database
- Curriculum from basic literacy through advanced engineering
