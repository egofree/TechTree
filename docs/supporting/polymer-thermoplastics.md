...
Process-level reference for thermoplastics relevant to the tech tree. Covers synthesis routes, processing conditions, properties, and key applications. Feedstocks come from [SQ 12](../side-quests/sq-12-petrochemicals.md); polymerization reactors and extruders rely on Phase 3 machine tools.
...
- [ ] **Phase 5**: LDPE production via high-pressure autoclave (requires pressure vessel capability)
- [ ] **Phase 5**: Suspension PVC from ethylene/chlorine feedstocks (ties to chlorine plant from Phase 5 chemistry)
- [ ] **Phase 5**: Crystal PS from styrene monomer (simplest thermoplastic to start with)
- [ ] **Phase 5**: Nylon 6,6 condensation polymerization (requires vacuum-capable autoclave)
- [ ] **Phase 5**: HDPE via Ziegler-Natta catalyst (requires TiCl₄ from Phase 5 chlorosilane chemistry)
- [ ] **Phase 5**: PTFE precursor chain (R-22 from chloroform + HF; HF available from Phase 5 fluorspar processing)
- [ ] **Phase 6**: PTFE sintering at 380°C (requires furnace control beyond typical polymer processing)
- [ ] **Phase 6+**: EPS foam production (requires pentane recovery and steam molding)
