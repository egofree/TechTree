...
|---------|---------------|----------|
| FDC alarm rate exceeds 5% false positives on etch chamber | Multivariate PCA model trained on insufficient baseline runs (<50); model threshold too tight (T² >95% instead of 99% confidence) | Retrain model with 50–100 baseline runs per chamber; widen statistical threshold to T² >99% confidence limit; review and exclude anomalous baseline runs caused by tool maintenance events |
| R2R controller drives CMP thickness further from target instead of closer | Process gain (b) value incorrect in EWMA algorithm — if actual gain is opposite sign or different magnitude, adjustment goes wrong direction; model mismatch after chamber hardware change | Re-estimate process gain (b) from recent DOE data; implement model health monitoring — track prediction error and auto-reset when error exceeds threshold; verify λ = 0.1–0.3 smoothing factor is appropriate for current drift rate |
| MES blocks lot from processing — "recipe not approved" error | Recipe version changed by process engineer but not yet approved through workflow (engineer → section manager → QA); or recipe checksum (CRC-32/MD5) failed during SECS/GEM S7F3 download | Complete approval workflow for the pending recipe version; if checksum failed, re-download recipe via SECS/GEM and verify; check that lot's product code matches recipe's approved product list |
| Wafer genealogy trace for defective die returns incomplete process history | FDC trace data missing for specific steps — tool was offline from SECS/GEM network during processing; or lot tracking RFID not scanned at track-in | Verify SECS/GEM communication at all critical tools; check lot track-in/track-out records for missing steps; investigate whether tool was in local mode (bypassing MES) during affected lots |
| SPC control chart shows out-of-control points but no actual yield impact | Control limits set too tight for the parameter's natural variation; or measurement tool drift causing apparent shift in readings | Recalculate control limits from recent 25–50 data points; verify measurement tool calibration; distinguish between special-cause variation (investigate) and over-sensitive limits (widen per Western Electric rules) |
| Rework loop percentage exceeds 10% of lots (normal: 2–10%) | Systematic process instability — multiple process steps drifting simultaneously; incoming material quality degraded; or rework criteria set too aggressively | Investigate root cause of rework increase via Pareto analysis; check incoming wafer and material quality; review rework criteria for over-triggering; if multiple process steps affected, check common-mode cause (facility utility excursion) |
| FDC server stops receiving trace data — no alarm raised | FDC health monitoring not implemented; SECS/GEM S6F1 trace collection link broken (network partition between tool and FDC server) | Implement FDC health monitoring — alarm if no trace data received for >60 seconds from any active tool; consider holding further processing on tools without FDC coverage until link restored |
| Dynamic routing sends lots to unqualified tool | MES qualification database not updated after chamber maintenance; tool dedication rules not enforced for specific process layer | Re-run qualification wafers after every maintenance event; verify MES tool qualification status is current; check tool dedication rules in MES — prevent incompatible lots from dispatching to dedicated tools |
| Batch formation delays lots >2 hours waiting for compatible companions | Too many small-lot products with incompatible recipes; batch timeout set too high; insufficient WIP to form full batches of 4–6 lots (100–150 wafers) | Reduce batch timeout to trigger partial batches sooner; review product mix for recipe compatibility grouping; consider adding a small-lot priority queue to bypass batch formation for urgent lots |
| Recipe download to tool rejected — "safety limit exceeded" | R2R controller computed an adjustment that pushed a parameter beyond the tool's hard safety limit (e.g., MFC full-scale range of 500 sccm); adjustment bypassed safety limit check | Verify all R2R adjustments pass through safety limit validation before SECS/GEM download; never allow R2R to modify safety limits — only process parameters within approved ±10–20% process window |
