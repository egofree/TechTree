...
2. **Immersion gold** (protective layer): Immerse the nickel-coated board in a gold bath (KAu(CN)₂, 1-3 g/L Au, pH 4.0-5.0, citrate buffer, 80-90°C). Gold displaces nickel from the surface by galvanic exchange: Au⁺ + Ni⁰ → Au⁰ + Ni²⁺. The gold deposits as a thin (0.03-0.08 μm) flash coating that protects the nickel from oxidation. The process is self-limiting — when all exposed nickel is covered, the reaction stops (no more nickel available for displacement). Time: 8-15 minutes.
