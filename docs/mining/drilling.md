...
> **Domain**: [Mining](./)
> **Dependencies**: `metals.iron-steel`, `machine-tools`
...
**Single jack drilling** (one person, one hammer):
...
**Double jack drilling** (two people, one sledge):
...
**Carbon steel bits** (standard for hand drilling and early machine drilling):
...
**Tungsten carbide insert bits** (for hard rock and pneumatic drills):
...
**Drifter drill** (mounted on a boom or carriage):
...
**Jackhammer blast holes** (small-scale underground):
...
**Long-hole drilling** (mechanized underground):
...
**Water flush** (simple, for short holes):
...
**Drilling mud** (for deep or unstable holes):
...
**Foam drilling** (alternative for dry holes):
...
**Acid etch survey** (simplest method):
...
**Abrasiveness** (determines bit wear):
...
**Tricone roller bit** (standard for large blast holes):
