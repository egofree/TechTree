...
|---------|---------------|----------|
| RED stack voltage below expected 0.1-0.2V per cell pair | Membrane fouling from river water contaminants; concentration polarization at membrane surfaces; incomplete wetting of membranes | Perform forward-flush at 10-20 cm/s for 5-10 minutes (2-4 times daily); verify pretreatment reduces TSS below 5 mg/L; check that membranes are fully hydrated — dry membranes have high resistance |
| Power output declining 20-50% over 1-3 months | Biofouling on membrane surfaces and spacer mesh; mineral scaling from calcium/magnesium in hard water | Implement periodic forward-flushing cycles; add NaOCl dosing at 1-3 mg/L residual chlorine for biofouling control (dechlorinate before stack if free chlorine exceeds 5 mg/L to protect AEM quaternary ammonium groups); replace degraded membrane pairs — economical at <$1/sq ft |
| Uneven flow distribution across membrane stack | Manifold ports misaligned during assembly; uneven stack compression (below specified 5-15 kPa) | Compress stack evenly using cross-pattern tightening with torque wrench to specified 5-15 kPa; verify alignment pins through all layers; check that spacer mesh is oriented correctly in each cell pair |
| Pumping energy exceeding 25% of gross power output | Narrow flow channels (100-300 μm) clogged with debris; pump oversized for actual flow requirements | Ensure pretreatment (sand filtration to below 5 mg/L TSS, turbidity below 1 NTU) removes fine particles; verify flow velocity at 1-5 cm/s in RED channels; match pump sizing to actual system head (5-20 m typical) |
| CEM and AEM membranes accidentally swapped during stack assembly | Visually identical membrane sheets; inadequate labeling during manufacturing (Step 4 vs Step 5) | Label all CEM and AEM membrane sheets clearly during manufacturing; assemble cell pairs in strict sequence: CEM → concentrate spacer → AEM → diluate spacer; a single reversed membrane pair cancels voltage contribution of adjacent cells |
| Saltwater corrosion of piping and fittings within weeks | Ferrous metals (steel, iron) or copper used in wetted components; PVC/CPVC not used throughout | Replace all wetted metal components with PVC, CPVC, HDPE, titanium, or fiberglass; PVC solvent-welded joints provide leak-tight seals without gaskets; isolate tie rods from electrode solutions with insulating bushings |
| Stack leaking at manifold connections | PVC cement joints not fully cured or improperly applied; uneven compression creating bypass flows between cell pairs | Apply PVC cement to both joining surfaces, press together with a quarter-turn, and allow full cure before pressurizing; torque stack evenly in cross-pattern; pressure-test each circuit (concentrate, diluate, electrode rinse) with fresh water before introducing salt solutions |
| Open-circuit voltage per cell pair below 0.1V with seawater/freshwater feed | Salinity ratio too low (diluate not fresh enough); membrane permselectivity below 0.90; air bubbles trapped in channels | Verify concentrate conductivity matches seawater (~35 g/L NaCl) and diluate matches fresh water (~0.5 g/L); purge air from all channels by flowing water at low rate before measuring; check for membrane defects by holding sheets against light for pinholes |
| Electrode rinse solution showing rapid color change (Fe²⁺/Fe³⁺ imbalance) | Electrode compartment not properly sealed; parasitic current through tie rods; electrode area undersized relative to membrane area | Verify electrode rinse circuit is isolated from concentrate/diluate circuits; insulate tie rods from electrode solutions; ensure electrode area matches active membrane area of outermost cell pair; circulate electrode rinse at low flow to purge air |
| Conductivity of outlet diluate not increasing (no ion transport) | Pumps not running or flow reversed; membranes installed with wrong orientation; stack not pressurized | Verify flow direction matches manifold labels (concentrate in → concentrate out, diluate in → diluate out); check pump operation and flow meter readings; ensure stack is compressed and all gaskets sealed; trapped air blocks channels and prevents ion transport |
