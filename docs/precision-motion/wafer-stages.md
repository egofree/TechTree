...
|---------|---------------|----------|
| MA (Moving Average) exceeds 2 nm during scan | Servo feedforward model drift; thermal expansion shifting stage calibration; interferometer wavelength error from atmospheric pressure change | Re-run stage grid calibration; verify atmospheric compensation sensor reading; check interferometer environmental sensor (temperature, pressure, humidity) is within spec; retrain feedforward model using last 50 scan overlays |
| MSD (Moving Standard Deviation) exceeds 1.5 nm | Structural resonance excited by scan trajectory; air bearing supply pressure fluctuation; external floor vibration | Check vibration isolation system status (air pressure, active isolation feedback); reduce servo bandwidth if approaching first resonance frequency; inspect air bearing supply for pressure regulator drift or filter clogging |
| Reticle-wafer synchronization error > 1 nm | Reticle stage servo gain mismatch; communication latency between stage controllers; reticle clamp not fully seated | Verify reticle clamp vacuum/force; re-calibrate reticle stage servo gains to match wafer stage; check controller sync clock and fiber-optic link integrity |
| Overlay registration drift between wafers | Stage mirror contamination or degradation; interferometer beam path obstruction (condensation, particles); thermal drift of stage structure | Clean interferometer mirrors with approved solvent and cleanroom wiper; check stage thermal management (cooling water flow and temperature); run stage grid calibration; verify cleanroom humidity is within spec to prevent condensation |
| Stage fails to reach synchronized speed before exposure | Scan acceleration ramp too aggressive for current payload; motor force constant degraded (demagnetized magnets); servo current limit reached | Reduce scan acceleration by 10-20% and retry; measure motor back-EMF to check for magnet degradation; verify power amplifier current delivery under load |
| Vacuum stage overheating in EUV mode | Cooling water flow restricted (clogged channels); motor heat exceeding conduction path capacity; outgassing deposits on thermal interfaces | Flush cooling channels with DI water; reduce stage duty cycle to allow thermal equilibration; check thermal interface materials for vacuum-induced delamination; verify water chiller temperature setpoint |
| Excessive particle counts in vacuum chamber | Outgassing from stage materials; wear debris from magnetic bearing sliding contact (during touchdown/startup); contaminated vent/purge gas | Run extended pump-down and bake-out cycle; inspect magnetic bearing touchdown surfaces for wear; replace vent/purge gas filter; verify all stage materials are on approved low-outgassing list |
| Focus drift during wafer exposure | Wafer chuck contamination (particles under wafer causing tilt); Z-axis piezo actuator creep; temperature gradient across wafer | Clean electrostatic chuck surface; run chuck flatness verification with test wafer; check Z-piezo preload and creep compensation table; verify wafer pre-alignment before chucking |
| Interferometer signal dropout or noise spikes | Air turbulence in beam path (for atmospheric stages); mirror surface contamination; laser source aging or mode hopping | For atmospheric stages: check beam path enclosures for leaks or temperature gradients; clean mirrors; check laser source power and wavelength stability; replace HeNe laser tube if output power has dropped >20% from rated |
| Stage resonance oscillation during step-and-settle | Servo bandwidth set too close to structural resonance; damping material degradation; loose mechanical joint in stage structure | Reduce servo bandwidth to ≤1/3 of first resonance peak; inspect structural joints for preload loss; check constrained-layer damping material condition (viscoelastic materials age and harden); run system identification to remap resonance frequencies |
