...
| Inventory records diverge from physical count | Receiving not recorded; partial issues not tracked; theft | Implement receiving inspection with mandatory ledger entry at gate; require signed issue slips for every withdrawal; investigate discrepancies >2% |
| Perishable goods spoiling in storage | FIFO not followed; storage conditions poor (damp, hot) | Reorganize warehouse for FIFO flow (in at back, out at front); monitor temperature and humidity; reduce order quantities for perishables |
| Price spikes from supplier collusion | Too few suppliers; no price transparency | Solicit bids from 3+ suppliers for every major input; publish reference prices from last 3 purchases; develop in-house production capability for highest-risk inputs |
| Transport capacity insufficient during peak demand | Seasonal demand surge (harvest, construction season) exceeds cart/mule availability | Pre-book transport capacity 1-2 months before peak; maintain community-owned cart fleet as backup; stagger production schedules to smooth demand |
