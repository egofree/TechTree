# SQ 5: Public Health, Sanitation &amp; Medicine

**Starts**: Phase 1+
**Priority**: High — skilled workers are the most valuable and irreplaceable resource.

## Problem

Industrial work is dangerous. Chemical processing (Phase 5+) involves lethal substances (HF acid, toxic gases, heavy metals). Mining, metalworking, and construction carry injury risks. Epidemics can decimate specialist populations. Losing experienced practitioners sets the project back years.
...
- **Driven wells** (Phase 3+): Drive steel pipe (3-5 cm diameter) with pointed screen tip into ground using sledge hammer or drop weight. Fast, works in sandy/gravelly soil. Depth 5-15 m. Attach hand pump.
...
- **Rapid sand filter** (Phase 5+): Coarser sand, higher flow rate (5-15 m/hour). Requires chemical pre-treatment (coagulation with alum — aluminum sulfate, then settling). Less biological, more physical filtration. Backwash with compressed air and water to clean.

**Chlorination** (Phase 5+):
...
- **Opium poppy (morphine)**: Score unripe seed pods. Collect dried latex (opium). Contains morphine (potent analgesic) and codeine (mild analgesic, cough suppressant). Extremely valuable for surgery and severe pain. Highly addictive — controlled use only.
  - **Morphine extraction** (Phase 5): Dissolve opium in hot water. Filter. Precipitate with NaOH → crude morphine base. Re-dissolve in HCl → morphine hydrochloride (water-soluble). Recrystallize for purity.
...
**Synthetic pharmaceuticals** (Phase 5+):
...
- **Phenol (carbolic acid)**: First surgical antiseptic (Lister, 1867). From coal tar distillation (see SQ 12). Dilute to 2-5% solution for wound irrigation and instrument sterilization. Pure phenol causes chemical burns — handle with care.
...
**Eye protection**:
- **Safety glasses**: Flat glass lenses (from Phase 2 glass production) in wire or stamped metal frames. Side shields from leather or sheet metal. For grinding and cutting operations — prevent flying particles.
...
- **Charcoal filter respirator** (Phase 5+): Canister filled with activated charcoal granules (wood charcoal heated to 800-900°C in steam → porous, enormous surface area). Charcoal adsorbs organic vapors and many toxic gases. Rubber or leather face mask with exhale valve. Replace canister when breathing becomes difficult or odor detected inside mask.
- **HF-specific protection**: Full-face shield + rubber apron + neoprene or nitrile gloves (from SQ 14 polymer production). Respiratory protection mandatory. Calcium gluconate gel (2.5%) stations at every HF work area — apply immediately to any skin exposure, massaging into affected area for 15+ minutes. HF penetrates skin, binds calcium → cardiac arrest. Even small exposures can be lethal without treatment.
...
- **Chemical-resistant gloves** (Phase 5+): Rubber (natural rubber latex or synthetic nitrile — see SQ 14). Thickness 0.2-0.5 mm for dexterity, 0.5-1.0 mm for chemical handling. Check for pinholes before each use (inflate with air, submerge in water — bubbles indicate holes).

**Body protection**:
- **Leather aprons**: For blacksmithing, foundry work, welding. Heavy cowhide, waist to knee length.
- **Rubber aprons** (Phase 5+): For chemical handling. Sheet rubber with neck loop and waist ties.
- **Cleanroom garments** (Phase 8): Woven synthetic coveralls, hoods, booties (from SQ 14 polymer fibers). Lint-free. Washed in ultra-pure water. Worn over street clothes. Changed every entry to cleanroom.
...
| Phase | Contribution |
|-------|-------------|
| Phase 1 | Basic hygiene, clean water, herbal medicine, wound care |
| Phase 2 | Mining injuries, burn treatment, metal fume fever, glass for eye protection |
| Phase 3 | Machine shop safety (entanglement, eye protection), precision instruments for medicine |
| Phase 4 | Boiler safety, electrical injury treatment, steam sterilization (autoclave) |
| Phase 5 | HF acid protocols, toxic gas safety, chemical burn treatment, synthetic pharmaceuticals |
| Phase 7-8 | Cleanroom health protocols, toxic dopant gas safety, ultra-pure water |
