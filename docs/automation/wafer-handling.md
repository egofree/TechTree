...
|---------|---------------|----------|
| Wafer broken during SCARA robot transfer (4–8 s cycle) | Acceleration exceeds 0.5 g (4.9 m/s²) limit — jerk-limited profile not applied; or end effector fingers worn past 500,000–1,000,000 cycle replacement interval | Verify S-curve velocity profile limits: max jerk 5,000 mm/s³, max acceleration 2,000 mm/s²; replace edge-grip fingers per scheduled replacement interval; check force sensing threshold ≤0.5 N above expected grip force |
| Particle count in ISO Class 1 chamber exceeds specification after robot operation | Sealed bearings on SCARA joints degraded — cleanroom grease (PFPE) depleted; or motor housing seal breached, shedding particles | Inspect and repack bearings with low-volatility PFPE grease; verify motor housing seal integrity; measure particle generation per cycle — must be ≤0.01 particles ≥0.1 μm per pick-place |
| Vacuum robot loses position after power failure — all teach points invalid | Controller lost non-volatile memory contents; checksum verification failed on power recovery; battery-backed SRAM depleted | Replace controller backup battery; re-teach all positions using calibration wafer with fiducial markers and camera system (2–4 hours downtime); implement checksum validation with auto-flag for position verification |
| Load lock pump-down time exceeds 90 seconds (design: 30–45 s) | Turbo pump (300 L/s) performance degraded; O-ring seal on slit valve leaking atmospheric gas into load lock volume (2–5 liters) | Check turbo pump rotation speed and current draw; perform leak check on slit valve Viton O-ring (replace if >500,000 cycles); verify load lock has no virtual leaks from trapped volumes under screws |
| Ultra-thin wafer (<200 μm) cracks in edge-grip end effector | Spring-loaded fingers exert excessive force (>2.0 N per finger) on fragile wafer edge; edge-grip not suitable for wafers below 200 μm thickness | Switch to Bernoulli (air levitation) end effector — zero physical contact via air bearing at 0.05–0.2 mm levitation height; supply clean dry air at 0.2–0.5 MPa, 5–20 L/min; Bernoulli works in atmospheric environment only |
| Wafer misaligned on pre-aligner chuck (±0.5 mm centering error) | Optical sensor for notch detection dirty or misaligned; pre-aligner chuck has particle contamination preventing flat wafer seating | Clean pre-aligner optical sensor; inspect and clean chuck surface; verify notch detection signal strength — 300 mm wafer U-shaped notch is 1 mm deep semi-circular |
| Slit valve metal seal (copper) fails to seal after <100,000 cycles | Copper gasket deformed unevenly from non-uniform bolt tightening; or sealing surface scratched from previous metal seal removal | Install metal seals with crisscross bolt pattern to 0.5–2.0 MPa compression; inspect sealing surfaces for scratches (finish <0.4 μm Ra); for ultra-high vacuum below 10⁻⁸ Torr, use new copper or aluminum seal each cycle |
| Dual-arm robot exchange move causes collision between arms | Scheduling conflict — both arms arrive at same chamber simultaneously; coordination software not enforcing minimum arm separation | Update cluster tool scheduler to enforce exclusive chamber access during exchange moves; verify exchange time budget (2–4 seconds) allows arm A to clear before arm B arrives; test with wafer simulants before production |
| Paddle end effector scratches wafer backside (surface roughness >0.5 μm Ra) | Paddle surface degraded — ceramic coating worn, exposing rough aluminum substrate; or particle trapped between paddle and wafer | Inspect paddle surface with profilometer (target <0.5 μm Ra); clean paddle with IPA wiper before each shift; replace paddle if coating damage visible; consider upgrading to Vespel or PEEK paddle material |
| Vacuum robot overheats during continuous operation — motor derated 50–70% in vacuum | Motor heat cannot convect away in vacuum; only conductive cooling path through robot structure to chamber walls is insufficient | Verify motor duty cycle does not exceed vacuum derating; check thermal modeling — add conductive heat sinking path from motor to chamber wall if needed; reduce continuous operation duty or add idle cooling periods |
