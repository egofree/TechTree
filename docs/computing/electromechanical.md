...
**[Pattern-following milling](../glossary/pattern-following-milling.html)** (tracing):
...
**[Punch card](../glossary/punch-card.html)** (for data and program storage):
...
**[Strowger switch](../glossary/strowger-switch.html)** (telephone switching):
...
**[Konrad Zuse Z3](../glossary/konrad-zuse-z3.html)** (1941, first working programmable computer):
...
**[Harvard Mark I](../glossary/harvard-mark-i.html)** (IBM ASCC, 1944):
...
**[Hollerith tabulating machine](../glossary/hollerith-tabulating-machine.html)** (1890 census):
...
**[Solenoids](../glossary/solenoids.html)** (linear actuators):
...
**[Stepper motors](../glossary/stepper-motors.html)** (incremental rotation):
...
**[Synchronous motors](../glossary/synchronous-motors.html)** (constant speed):
...
**[Watt-hour meter](../glossary/watt-hour-meter.html)** (energy measurement):
...
**[Wire recording](../glossary/wire-recording.html)** (early audio and data storage):
...
- **[Keypunch](../glossary/keypunch.html)** (IBM 029, standard model): Operator types on keyboard, machine punches holes at 10 characters per second. Each keystroke punches one column and advances to the next. Verification mode: a second operator re-keys the same data, and the machine compares punches to keystrokes without punching, flagging mismatches.
- **[Sorter](../glossary/sorter.html)** (IBM 082): Reads one selected column per pass. Drops each card into one of 13 pockets (digits 0-9, zones 11/12, reject). Throughput: 450 cards per minute. Multi-digit sorting requires one pass per digit position, sorting least significant digit first. Sorting 10,000 cards on a 5-digit field: 5 passes × 22 minutes per pass ≈ 2 hours.
- **[Tabulator](../glossary/tabulator.html)** (IBM 407): Reads cards at 150 per minute. Performs addition and subtraction on up to 20 counters (each up to 8 digits). Prints one line per card or per control group, with selective printing based on punch patterns. Program control via removable wiring panel (plugboard) that defines which columns feed which counters and when printing occurs.
