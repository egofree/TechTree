...
**Arc flash hazard**: Short-circuit current in 480 V systems can reach 20-65 kA. Arc temperature: 20,000°C (4× surface of sun). Arc blast pressure: 500-2000 lb/ft² at 0.6 m. Incident energy: 1-100+ cal/cm² (1.2 cal/cm² causes second-degree burn). To calculate incident energy at a working distance: use IEEE 1584 method. For a 1000 kVA transformer at 480 V with 5.75% impedance (I_sc = 20,900 A symmetrical), at 0.6 m working distance with 0.2 second clearing time: incident energy ≈ 12 cal/cm². PPE required: arc-rated clothing (8-100 cal/cm² depending on incident energy calculation), face shield with arc-rated lens, voltage-rated gloves (Class 00 = 500 V AC, Class 0 = 1000 V AC), hearing protection. Arc flash boundary: distance where incident energy = 1.2 cal/cm² (typically 0.3-3.0 m from equipment). This boundary must be marked on equipment with a warning label listing incident energy, arc flash boundary, and required PPE.

**Lockout/tagout (LOTO)**: Before working on energized equipment: (1) identify all energy sources (electrical, mechanical, hydraulic, pneumatic, thermal, chemical), (2) notify affected personnel, (3) shut down equipment by normal operating procedure, (4) isolate and lock each energy source with individual padlock (rated for the application, keyed differently for each worker), (5) apply tag with worker name, date, and reason, (6) verify zero energy state (test with voltage detector rated for the circuit voltage: contact-type for <600 V, non-contact proximity for >600 V), (7) perform work, (8) remove locks and tags (only by the worker who placed them), (9) notify personnel of re-energization. Each worker applies their own lock. No one may remove another's lock. If a worker leaves the site, their lock is removed only by a supervisor who has verified the worker is clear of the equipment and documented the removal.

**Insulation testing**: Megger (insulation resistance tester) applies 500-10,000 V DC and measures leakage current. Minimum acceptable insulation resistance: 1 MΩ per kV of operating voltage + 1 MΩ (e.g., 2 MΩ for a 1 kV motor winding at 500 V DC test voltage). Polarization index (PI) = resistance at 10 min / resistance at 1 min. PI >2.0 indicates good insulation; PI 1.5-2.0 indicates marginal insulation requiring monitoring; PI <1.5 indicates deterioration requiring immediate investigation. The 10-minute test must be performed on de-energized equipment with all external connections disconnected. Discharge the winding capacitance through a 10-100 kΩ resistor for at least 1 minute after testing before reconnecting.
