...
|---------|---------------|----------|
| Type K thermocouple reads 15°C high at 800°C | Thermocouple wire drifted from prolonged exposure above 1,000°C — Seebeck coefficient shifts with grain growth and selective oxidation in Chromel/Alumel | Replace Type K element; for furnace work above 1,000°C, use Type S (Pt-10%Rh/Pt, ±1.0°C) for calibration and limit Type K to general-purpose use below 1,000°C |
| Pt100 RTD reads 0.4°C low consistently at 100°C | Two-wire connection instead of four-wire (Kelvin) — lead resistance adds error; each 0.385 Ω of lead resistance = 1°C error at the Pt100's 0.385 Ω/°C coefficient | Switch to four-wire (Kelvin) measurement — constant current source drives sensor, measure voltage drop independently, eliminating lead resistance from the measurement |
| Bourdon tube pressure gauge reads 3% high at full scale, passes zero correctly | Sector gear connecting link rod adjusted too short — over-amplifies tube tip motion; or tube permanently deformed from previous overpressure event | Adjust link rod length (threaded rod with locknuts) to set full-scale span; if adjustment cannot correct, replace gauge — permanent deformation from exceeding 1.5× MAWP weakens burst resistance |
| Mercury barometer column drops below 700 mm at sea level | Air leaked into tube during inversion step — dissolved gas in mercury or incomplete fill creates gas pocket above Torricelli vacuum; leak at sealed end | Drain, refill with degassed mercury (warm to 50°C, tap to release bubbles, repeat 3–4 cycles); verify sealed end has no pinhole by pressurizing with rubber bulb and submerging in water |
| Thermocouple reads correctly at room temperature but drifts at furnace temperature | Cold junction compensation not applied or incorrect reference junction temperature measured; or wrong thermocouple type selected for readout instrument | Verify thermocouple type matches instrument input setting; measure reference junction temperature independently; apply cold junction correction per NIST thermocouple tables for the specific type |
| Bimetallic thermometer reads 8°C low after 6 months of service at 400°C | Bimetallic bond separating or strip permanently deformed from overheating near its 500°C maximum; differential expansion characteristic altered | Replace bimetallic element — permanent drift from thermal fatigue is not correctable; for sustained operation above 400°C, switch to thermocouple or pyrometer measurement |
| Liquid-in-glass thermometer shows non-linear readings between 0°C and 100°C marks | Capillary bore inconsistent (>5% variation) — bore constrictions cause mercury to move non-uniformly; tube not rejected during selection | Discard and redraw capillary tube — bore must be uniform within 5%; verify by drawing a short mercury thread through and checking for sticking or speed variations at different positions |
| Two-color pyrometer reads 50°C high on silicon wafers in CVD reactor | Emissivity variation during deposition — even ratio pyrometers can err when emissivity changes differently at the two measurement wavelengths during film growth | Calibrate against embedded thermocouple during initial qualification; use known emissivity at both wavelengths; for CVD applications, consider using lamp thermometry or optical fiber temperature measurement |
| Inclined manometer reading drifts 0.05 mbar over 8 hours | Temperature change alters fill fluid density — water density changes ~0.02%/°C; room temperature not stable; no thermal shielding on manometer | Install manometer in temperature-controlled location (±1°C); use oil fill (lower thermal expansion coefficient than water) for better stability; apply temperature correction to density calculation |
| U-tube manometer shows asymmetric liquid levels when both sides open to atmosphere | Liquid holdup on one side from surface tension or contamination in tube; or manometer not vertically aligned (use plumb line to verify) | Clean tube interior with solvent; verify vertical alignment with plumb line — even a 2° tilt causes measurable error; ensure both ends see the same reference pressure before zeroing |
