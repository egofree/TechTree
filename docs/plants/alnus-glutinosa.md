# Black Alder

> **Node ID**: plants.timber.black-alder
> **Domain**: [Plants](./index.md)
> **Dependencies**: See prerequisites
> **Enables**: [`Construction`](../construction/index.md), [`Agriculture`](../agriculture/index.md)
> **Timeline**: Years 2-15
> **Outputs**: water-resistant timber, tannin bark, nitrogen fixation
> **Critical**: No — specialized underwater construction timber but not irreplaceable

## Overview

![Black Alder](../images/plants/plants_alnus-glutinosa.jpg)

> *A twig of black alder (Alnus glutinosa), a piece of deadwood, and water ripples formed by a rising gas bubble. Photographed at the Wolfsberger Mühle on the Reppeliner Bach, about two kilometers north of Tessin in the Rostock district, within the Stormsdorfer Bachtal nature reserve.*

> *Image: Radomianin, CC BY-SA 4.0*

Black alder (*Alnus glutinosa*) is a deciduous tree native to Europe that produces timber with a remarkable property: it is highly resistant to decay when permanently submerged in water or buried in wet ground. Alder piles driven into riverbeds and marshes can last for centuries. Venice was built on alder pilings, and many medieval structures in northern Europe rest on alder foundations.

The wood is relatively soft and light (450-550 kg/m³ at 12% moisture content) but becomes extremely hard and durable when permanently wet. This paradox (soft when dry, durable when wet) makes alder uniquely suited for foundation pilings, underwater structures, sluice gates, and any construction that contacts water or damp ground.

Alder is also a nitrogen-fixing tree, hosting Frankia bacteria in root nodules that convert atmospheric nitrogen to plant-available forms. A stand of alder adds 50-150 kg of nitrogen per hectare per year to the soil. This makes it valuable for reclaiming degraded land, improving soil before planting other crops, and stabilizing stream banks.

The bark contains 8-16% tannins, usable for leather tanning though inferior to oak or wattle bark. The bark was also used historically to produce red-brown and black dyes.

The tree grows 10-25 meters tall, thrives in wet soils and along waterways, and reaches harvestable size in 15-25 years. Alder regrows vigorously from cut stumps (coppices), providing a sustained yield of poles and timber on a 10-15 year cycle.

Primary outputs: `water_resistant_timber` for underwater construction, `nitrogen_fixation` for soil improvement, `tannin_bark` for leather and dye.

## Prerequisites

### Materials

- Alder seeds or bare-root seedlings
- Wet or moist site: riverbank, marsh edge, or poorly drained soil
- Clay, loam, or peat soils (tolerates waterlogging better than most trees)

### Equipment

- Felling axe, crosscut saw, or chainsaw
- Driving cap (heavy metal ring) and maul for pile driving
- Drawknife for bark stripping
- Basic hand tools for woodworking

### Knowledge

- Alder identification (round leaves with notched tips, cone-like fruit)
- Pile driving technique for foundation work
- Coppice management for sustained production
- Nitrogen fixation benefits for crop rotation planning

### Infrastructure

- Wet site planting area
- Pile driving equipment for construction use
- Coppice woodland management plan

## Process Description

### Timber Production

1. Plant seedlings or direct-sow seeds in moist to wet soil. Space 2-3 meters apart for timber, 1-1.5 meters for coppice.
2. Allow trees to grow for 15-25 years for structural timber, or coppice on 10-15 year cycles for poles.
3. Fell during winter when sap is down. Alder fells and works more easily than oak or ash.
4. For underwater use: cut trunks into piles of required length (3-10 meters). Remove bark (optional, as bark also resists decay underwater). Shape the base to a point for driving.
5. Drive piles into the riverbed or marsh using a drop hammer or maul. The pile should penetrate below the water table into firm substrate.
6. Once driven, the pile is permanently submerged and will resist decay for 100+ years.

### Nitrogen Fixation for Soil Improvement

1. Plant alder on degraded, nitrogen-poor soils. The tree grows where few other species survive.
2. Allow 5-10 years of growth. During this period, the root nodules fix 50-150 kg of nitrogen per hectare per year.
3. Clear the alder (harvest timber and fuel) and plant crops in the nitrogen-enriched soil. The nitrogen is released as the alder roots and leaf litter decompose.
4. Alternatively, interplant alder with other trees in a mixed woodland to maintain soil fertility.

### Process Parameters

| Parameter | Range | Notes |
|-----------|-------|-------|
| Wood density (12% MC) | 450-550 kg/m³ | Light to moderate |
| Bending strength (MOR) | 65-85 MPa | Moderate |
| Compression parallel to grain | 35-45 MPa | Moderate |
| Natural durability (dry, above ground) | Class 5 (not durable) | Decays rapidly if dry and exposed |
| Natural durability (permanently wet) | Excellent | 100+ years underwater |
| Nitrogen fixation rate | 50-150 kg/ha/year | Via root nodules |
| Tannin content (bark) | 8-16% | Usable but not premium |
| Tree height at maturity | 10-25 m | 15-25 years |
| Coppice rotation | 10-15 years | For poles and small timber |
| Optimal site | Wet soils, riverbanks | Tolerates waterlogging |
| Growth rate | 0.5-1.0 m/year | In good conditions |
| Soil nitrogen addition | 50-150 kg/ha/year | Via Frankia root nodules |
| Bark tannin yield | 30-80 kg/ha | At 15-25 year harvest |
| Timber yield per hectare | 5-15 m³ | At 15-25 year rotation |

### Timber Processing Details

Alder wood has specific processing requirements that differ from other hardwoods. Fresh-cut alder has a high moisture content (70-100% of dry weight) and must be dried carefully to avoid severe warping and checking. The wood has high volumetric shrinkage (11-13%), meaning it moves significantly during drying. Stack alder boards with close sticker spacing (30 cm) and weight the stack to restrain movement.

A notable characteristic of fresh alder wood: it turns a distinctive orange-red color when exposed to air and light. This color change is harmless and stabilizes within hours. The dried wood is pale reddish-brown with a fine, even texture that finishes smoothly with hand tools.

For underwater pilings, the critical factor is ensuring the pile remains permanently below the water table. Alder that cycles between wet and dry conditions will decay rapidly. The transition zone at the waterline is the most vulnerable point. Traditional practice extends the pile several meters below the lowest recorded water level and caps the top with a protective band of oak, copper sheet, or concrete.

### Nitrogen Fixation in Practice

Alder root nodules host Frankia actinomycetes (not rhizobia bacteria like legumes). These nodules are visible as clusters of coral-like growths on fine roots, typically 2-5 mm in diameter. Actively fixing nodules are pink to red inside due to leghemoglobin, which regulates oxygen supply to the nitrogenase enzyme.

A stand of 2,000 alder trees per hectare can add 50-150 kg of nitrogen annually to the soil through leaf litter, root turnover, and nodule senescence. This is equivalent to applying 100-300 kg of ammonium sulfate fertilizer per hectare. When alder is cleared for crop planting, the decomposing roots and leaf litter release this stored nitrogen gradually over 2-3 years, providing a sustained nutrient supply for the following crop.

## Safety Considerations

- Pile driving is hazardous. Heavy piles and driving equipment can cause crushing injuries. Use mechanical assistance. Never stand under a raised pile.
- Alder wood dust can cause respiratory irritation. Wear a mask when sawing or sanding.
- Working in wet conditions (typical for alder harvest) increases risks of slips, falls, and hypothermia. Wear appropriate footwear and clothing.
- Felling trees near waterways creates falling hazards into water. Plan felling direction carefully.

### Personal Protective Equipment

- Steel-toed waterproof boots for wet-site work
- Hard hat for pile driving and felling
- Safety glasses for sawing and woodworking
- Dust mask for sanding operations

## Quality Control

### Acceptance Criteria

- **Foundation piles**: Straight, minimum 15 cm diameter at butt, bark removed, base pointed for driving, no heartshake or large knots
- **Coppice poles**: Straight, minimum 5 cm diameter, no rot, even taper
- **Bark for tanning**: Inner surface orange-brown, no rot, no wood adhering

### Testing Methods

- Pile quality: tap the butt end with a hammer. A clear ring indicates sound wood. A dull thud indicates internal decay.
- Water resistance verification: cut a cross-section and soak in water for 30 days. Sound alder shows no softening or decay.
- Nitrogen fixation: dig up roots and check for visible nodules (small, coral-like growths on roots). Active nodules are pink inside.

## Scaling Notes

- **Individual tree**: Produces one pile (3-10 m long) or several poles.
- **Riparian strip** (1 hectare): 2,000-5,000 trees. Annual timber yield: 10-30 cubic meters from coppice rotation. Nitrogen input: 50-150 kg per year.
- **Foundation project**: A modest building on wet ground requires 50-200 piles, each 3-6 meters long. This requires 50-200 mature alder trees.

## Troubleshooting

| Problem | Probable Cause | Solution |
|---------|---------------|----------|
| Piles decay after driving | Not driven below permanent water table | Drive deeper; ensure pile is always submerged |
| Alder grows poorly on dry site | Species adapted to wet conditions; drought stress | Plant in moist or wet locations; irrigate during establishment |
| Coppice shoots die back | Fungal infection (Phytophthora) or frost damage | Improve drainage; select frost-resistant individuals |
| Wood warps badly during drying | Rapid, uneven drying | Stack with close stickers; weight the stack; dry slowly |

## Variations and Alternatives

- **Red alder** (*Alnus rubra*): North American species with similar properties. Grows faster in the Pacific Northwest.
- **Italian alder** (*Alnus cordata*): Tolerates drier conditions. Good for land reclamation in Mediterranean climates.
- **Elm** (*Ulmus minor*): Also water-resistant and used for underwater structures, though elm is heavier and harder to work.
- **Oak** (*Quercus robur*): Heartwood is durable in ground contact but less so than alder when fully submerged. Much stronger but slower growing.
- **White cedar** (*Thuja occidentalis*): Naturally durable in ground contact and water. North American species. Lighter but weaker than alder.
- **Concrete piles**: At higher technology levels, concrete replaces wooden pilings entirely. But for early-stage construction, alder provides the most accessible water-resistant foundation material.

### Dye and Tannin Production

Alder bark produces a warm brown dye when boiled in water. Adding iron (rusty nails or iron water) shifts the color to deep brown-black. The bark contains 8-16% tannins, enough for leather tanning in regions where oak or wattle is unavailable. The tanning process is identical to other bark tanning: steep bark in water, prepare hides, and immerse for 2-8 weeks. Alder-tanned leather has a distinctive reddish-brown color and is softer than wattle-tanned leather, making it suitable for garment leather rather than heavy harness leather.

## References

- [Construction](../construction/index.md) — foundation building in wet conditions
- [Agriculture](../agriculture/index.md) — nitrogen fixation and soil improvement
- [English Oak](./quercus-robur.md) — comparison timber for construction
- [Plants Domain](./index.md) — domain overview and related capabilities

### Alder in Agroforestry Systems

Alder's nitrogen-fixing ability makes it a key component of temperate agroforestry systems.
In traditional European woodland management, alder was maintained along waterways and in wet
areas of coppice woodland, providing poles for fencing and tool handles while enriching the
soil for neighboring trees. A mixed alder-oak woodland produces better oak growth than pure
oak stands because the alder's nitrogen fixation fertilizes the oak trees.

For land reclamation, alder is planted as a pioneer species on degraded sites: former industrial
land, mining spoil heaps, and eroded hillsides. The tree establishes quickly, improves soil
structure and fertility, and creates conditions suitable for other tree species to colonize.
After 10-15 years of alder growth, the site can be interplanted with climax species (oak, beech,
ash) that will eventually replace the alder as the canopy closes.

### Alder Charcoal and Fuelwood

Alder coppice on a 10-15 year rotation produces good fuelwood. The wood burns steadily with a
moderate heat output, suitable for domestic heating and cooking. Alder charcoal is soft and
lights easily but burns faster than hardwood charcoal — suitable for cooking but not preferred
for smelting. The charcoal was historically used in gunpowder manufacture because it ignites
reliably at a consistent rate.

Alder wood turns well on a lathe and takes a smooth finish, making it suitable for turned items like bowls, tool handles, and chair legs. Fresh-cut alder has a bright orange-red color that fades to pale brown upon exposure to air. The wood accepts stain and paint readily, and in historic Europe it was often used as a cheaper alternative to mahogany for furniture that would be painted or stained.

### Black Alder Summary

This species represents an important component of a diversified food production system.
No single crop provides complete nutrition, and dietary diversity is essential for human
health. A civilization bootstrap food system should include grains, legumes, roots, fruits,
vegetables, and nuts to ensure adequate intake of calories, protein, vitamins, and minerals.

The crop's specific growing requirements (soil type, rainfall, temperature range, and
growing season length) determine its geographic suitability. Matching crops to local

---
*Part of the [Bootciv Tech Tree](../index.md) · [Plants](./index.md) · [All Domains](../index.md)*
