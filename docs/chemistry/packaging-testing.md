...
**Encapsulation**:
- **[Transfer molding](../glossary/transfer-molding.html)** (most common for plastic packages): Place die + wire-bonded lead frame in mold cavity. Inject molding compound (epoxy resin + silica filler 70-80% by weight + catalyst + mold release agent) at 175°C, 6-10 MPa pressure. Cure 2-5 minutes. Mold compound protects die from moisture, mechanical damage, contamination. Silica filler reduces thermal expansion coefficient (match silicon → less stress).
- **[Ceramic hermetic packages](../glossary/ceramic-hermetic-packages.html)** (high-reliability): Multi-layer ceramic (alumina) with tungsten metallization traces. Co-fired at 1500-1600°C. Seal lid to package with gold-tin eutectic solder (Au-20Sn, mp 280°C) in dry nitrogen atmosphere. Hermetic (gas-tight) — moisture cannot penetrate. For military, aerospace, high-temperature applications.
...
**[Wafer probing](../glossary/wafer-probing.html)** (test before packaging):
...
**Failure analysis techniques**: (1) **[Optical microscopy](../glossary/optical-microscopy.html)** and **[SEM](../glossary/sem.html)** for visual inspection. (2) **[Acoustic microscopy](../glossary/acoustic-microscopy.html)** (C-SAM) for delamination and void detection — non-destructive. (3) **Decapsulation**: acid (fuming HNO₃ or H₂SO₄) or laser to remove package molding compound and expose die for analysis. (4) **FIB (focused ion beam)**: cross-section specific features for SEM inspection, or cut circuit traces for circuit edit. (5) **EMMI (emission microscopy)**: detects photon emission from defective transistors (hot carrier luminescence, avalanche breakdown). (6) **OBIRCH (optical beam induced resistance change)**: laser scanning detects thermal response changes — localizes shorts and leakage paths.

### Reliability Standards

**[JEDEC](../glossary/jedec.html)** standards define test methods and acceptance criteria: JESD22 (test methods), JESD47 (stress-test-driven qualification), JESD74 (early life failure rate). **[AEC-Q100](../glossary/aec-q100.html)** for automotive electronics: more stringent than JEDEC — temperature range -40 to +150°C (Grade 0), zero defects philosophy, additional tests (electromigration, soft error rate). **[MIL-STD-883](../glossary/mil-std-883.html)** for military/space: even more stringent — radiation testing (total ionizing dose, single event effects), outgassing testing (ASTM E595, <1% total mass loss, <0.1% collected volatile condensable materials for spacecraft materials).
